...
namespace __hidden_skinned_geometry_runtime_mesh_cache_resources{
...
[[nodiscard]] bool ValidateRuntimeMeshUploadPayload(Core::Alloc::GlobalArena& arena, const SkinnedGeometryRuntimeMeshInstance& instance){
    ...
    if(
        ...
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: runtime mesh '{}' has incomplete split geometry payload")
            , TStringView(sourceText)
        );
        return false;
    }
    if(
        ...
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: runtime mesh '{}' exceeds u32 payload limits")
            , TStringView(sourceText)
        );
        return false;
    }
    if(instance.skeletonJointCount == 0u){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: runtime mesh '{}' has skin but no skeleton joint count")
            , TStringView(sourceText)
        );
        return false;
    }
    if(!SkinnedGeometryValidation::ValidInverseBindMatrices(instance.inverseBindMatrices, instance.skeletonJointCount)){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: runtime mesh '{}' inverse bind matrices are invalid")
            , TStringView(sourceText)
        );
        return false;
    }
    ...
    for(usize skinIndex = 0u; skinIndex < instance.skin.size(); ++skinIndex){
        ...
        u32 failedJoint = 0u;
        if(
            SkinnedGeometryValidation::ValidSkinInfluence(skin)
            && SkinnedGeometryValidation::SkinInfluenceFitsSkeleton(skin, instance.skeletonJointCount, failedJoint)
        )
            continue;

        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: runtime mesh '{}' skin influence {} is invalid")
            , TStringView(sourceText)
            , skinIndex
        );
        return false;
    ...
    for(usize vertexRefIndex = 0u; vertexRefIndex < instance.vertexRefs.size(); ++vertexRefIndex){
        ...
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: runtime mesh '{}' vertex ref {} is out of range")
            , TStringView(sourceText)
            , vertexRefIndex
        );
        return false;
...
[[nodiscard]] Core::BufferHandle SetupRuntimeBuffer(
    ...
    if(payload.empty()){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: {} payload is empty"), label);
        return {};
    }
    if(!RuntimeGeometryBufferUpload::PayloadByteCountFits<PayloadT>(payload)){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: {} payload byte size overflows"), label);
        return {};
    }
...
[[nodiscard]] Core::BufferHandle SetupRuntimeBuffer(
    ...
    if(!bufferName){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: failed to derive {} buffer name for runtime mesh '{}'")
            , label
    ...
    NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometryRuntimeMeshCache: failed to create {} buffer for runtime mesh '{}'")
        , label
        , instance.handle.value
    );
    return {};
...
bool SkinnedGeometryRuntimeMeshCache::uploadRuntimeMeshBuffers(SkinnedGeometryRuntimeMeshInstance& instance){
    if(!__hidden_skinned_geometry_runtime_mesh_cache_resources::ValidateRuntimeMeshUploadPayload(m_arena, instance))
        return false;

    bool uploaded = true;
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<Float3U>(
        m_graphics,
        instance,
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<Half4U>(
        m_graphics,
        instance,
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<Float2U>(
        m_graphics,
        instance,
        instance.uv0Buffer,
        AStringView("uv0"),
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<GeometryVertexRef>(
        m_graphics,
        instance,
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<GeometryMeshletDesc>(
        m_graphics,
        instance,
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<GeometryMeshletBounds>(
        m_graphics,
        instance,
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<u32>(
        m_graphics,
        instance,
    ...
    uploaded = __hidden_skinned_geometry_runtime_mesh_cache_resources::AssignRuntimeBuffer<u8>(
        m_graphics,
        instance,
