...
- Do not hide a function return with `static_cast<void>(call())`, a C-style void cast, or a `[[maybe_unused]]` local initialized from the call — handle it. Check and act: `if(!call(...)){ log / propagate / early-return }`, or fold the discarded `bool` into the matching out-error check (`static_cast<void>(f(p, error)); if(error) ...` → `if(!f(p, error)) ...`).
