...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  photon producer, geometry downsample, resolve-prepare, first-wavelet, and timed tail are distinct merged callbacks
  ...
  `UnorderedAccess`; and first-wavelet reads that target as `ShaderResource` while writing the counterpart as
  `UnorderedAccess`. The compiler therefore lowers the photon/geometry and first ping-pong in-packet
  `UnorderedAccess`-to-`ShaderResource` handoffs before the consuming callbacks record; the tail closes the retained
  interval. When no photon producer records, geometry, prepare, first-wavelet, and tail emit no dispatch and the
  graph-owned black irradiance clear remains final. Packet-identity validation retains the established timing and
  acceptance endpoint. Direct callers retain native setup by default, while only the remaining alternating
  history/half passes, upsample, and their local UAV ordering remain native task-local ownership. The normal graph
  also no longer repeats unchanged world/depth sampled-state setup across each wavelet pass.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  reasserting them natively. The selected photon producer, geometry downsample, resolve-prepare, first-wavelet, and
  timed tail are distinct merged callbacks in that same caustics packet: geometry starts the retained full-resolve
  timing interval; prepare declares the exact accumulator, geometry cache, and parity-selected ping-pong input as
  `ShaderResource` plus its target as `UnorderedAccess`; and first-wavelet reads that target as `ShaderResource`
  while writing the counterpart as `UnorderedAccess`. The compiler therefore lowers the photon/geometry and first
  ping-pong in-packet `UnorderedAccess`-to-`ShaderResource` handoffs before the consuming callbacks record; the tail
  closes the retained interval. When no photon producer records, geometry, prepare, first-wavelet, and tail emit no
  dispatch and the graph-owned black irradiance clear remains final. Packet-identity validation retains the
  established timing and acceptance endpoint. Direct callers retain native setup by default; only the remaining
  alternating history/half passes, upsample, and their local UAV ordering remain native task-local ownership. The
  normal graph also no longer repeats unchanged world/depth sampled-state setup across each wavelet pass.
