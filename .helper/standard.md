...
- Represent expected runtime failures with explicit return values or status objects. Do not throw and catch an exception to continue normal application execution.
...
- `global/` is the neutral global-unit domain (global scope, shared layouts, process-wide contracts), not the owner of the `NWB` namespace wrapper. `global/namespace.h` only hosts the `NWB_BEGIN`/`NWB_END` macro definitions there because every domain includes `global/` first; keep those macros defined (not namespace-wrapped) in that one header and do not scatter duplicate definitions.
