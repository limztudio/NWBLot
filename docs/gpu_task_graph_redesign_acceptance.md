...
- The opaque G-buffer now freezes its material draw ordering and CSG CPU frame payload during graph declaration.
  ...
  (`csgCapBackNormal`, `csgIntervalDepth`, and `csgIntervalId`) plus receiver-surface `csgReceiverEventData` now
  declare their exact `UnorderedAccess` ranges before normal opaque and prepared-transparent thunks record, so those
  producers no longer stage their initial StorageImage states natively. The later native combine barrier, wider CSG
