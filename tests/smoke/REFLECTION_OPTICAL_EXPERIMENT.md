...
**The full ECS suite was not clean at experiment completion.** The candidate run passed 315 of 359 tests, with 44 failures and 61 disabled tests. The pulled baseline passed 314 of 358 with the exact same 44 failing names; the extra passing test is the added no-CSG regression. Source-layout/ownership contracts had not all been migrated after extraction. One confirmed pre-existing boundary issue was the deferred frame-tail builder's direct dependency on RendererFramePipeline. The matching failure set did not establish that every existing failure was harmless. The subsequent [renderer error cleanup](../RENDERER_VALIDATION.md) repairs these failures, including the ownership issue, and records its separate validation results; the experiment results above remain unchanged.
