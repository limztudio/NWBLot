...
- Add a narrower exception boundary only when the boundary is structurally required, such as translating an exception across a worker-thread or C ABI boundary, restoring an invariant before rethrow, or performing terminal shutdown. Never swallow an exception and continue from such a boundary.
