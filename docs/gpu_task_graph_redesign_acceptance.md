...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  caustics packet. The initialized mirror publishes only after that packet's producer accepts. Direct callers
  retain native setup by default, while non-temporal reset, later temporal decay, and the multi-pass resolve/UAV
  sequence remain native task-local ownership.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  clear at the front of the same caustics packet. The initialized mirror publishes only after that packet's producer
  accepts. Direct callers retain native setup by default; non-temporal reset, later temporal decay, and the
