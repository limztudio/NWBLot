...
bool LoadScene(const ImportOptions& options, SceneHandle& outScene, AString& outError){
    ...
    if(!outScene.scene){
        outError = "failed to load FBX: " + __hidden_fbx_scene::FormatUfbxError(error);
        return false;
    }
...
void PrintMeshInstances(const UtilityVector<MeshInstance>& instances){
    ...
    for(const MeshInstance& instance : instances)
        NWB_COUT << "  " << __hidden_fbx_scene::MeshDisplayName(instance) << "\n";
}
...
bool SelectMeshInstances(
    ...
    if(__hidden_fbx_scene::ParseIndexSelector(normalized, parsedIndex)){
    ...
    for(const MeshInstance& instance : instances){
        if(
            __hidden_fbx_scene::NormalizedAsciiEqual(instance.node->name, normalized)
            || __hidden_fbx_scene::NormalizedAsciiEqual(instance.mesh->name, normalized)
        ){
            ...
            outSelection.empty()
            && (
                __hidden_fbx_scene::NormalizedAsciiContains(instance.node->name, normalized)
                || __hidden_fbx_scene::NormalizedAsciiContains(instance.mesh->name, normalized)
            )
        ){
