...
- `ab/` holds manually launched A/B measurement workflows. Each runnable workflow has a terminal `launcher.py`; capture, timing, and result artifacts stay local under `.cozter/out/ab-results/`.

CTest target names remain stable across this layout. Source-only CTest directories do not become launcher commands; add a terminal `launcher.py` only for an independently runnable workflow. The current root-launcher A/B commands are `async-shadow-m4`, `command-ir`, `frame-lagged-async-lighting`, `hybrid-shadow-boundary`, and `transfer-queue`.
