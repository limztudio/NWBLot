...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  `ShaderResource` and resolve-geometry `UnorderedAccess` states without reasserting them natively. A merged shared
  resolver task then declares the exact accumulator range as `ShaderResource`, so the compiler lowers the photon
  producer's in-packet `UnorderedAccess`-to-`ShaderResource` handoff; when no photon producer records, that resolver
  emits no dispatch and the graph-owned black irradiance clear remains final. Packet-identity validation retains the
  established timing and acceptance endpoint. Direct callers retain native setup by default, while later dynamic
  resolve-buffer transitions remain native task-local ownership. The normal graph also no longer repeats unchanged
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  reasserting them natively. A merged shared resolver task then declares the exact accumulator range as
  `ShaderResource`, so the compiler lowers the photon producer's in-packet `UnorderedAccess`-to-`ShaderResource`
  handoff; when no photon producer records, that resolver emits no dispatch and the graph-owned black irradiance
  clear remains final. Packet-identity validation retains the established timing and acceptance endpoint. Direct
  callers retain native setup by default; later dynamic resolve/ping-pong transitions remain native task-local
