...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  closes that interval. Direct callers retain native setup by default, while the multi-pass resolve/UAV sequence
  remains native task-local ownership.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  receives the compiler-owned same-UAV fence and closes that interval. Direct callers retain native setup by default;
  the multi-pass resolve/ping-pong transitions remain native task-local ownership.
