...
| 10 | Test a separately compiled optical kernel for proven air origins | Evaluated and rejected; strong clear-scene observation, but control drift and unresolved inside overhead fail retention gates |
...
## Step 8: preserve rejection order for derivative-dependent surface work

The proposed optimization would discard the selected primary refraction instance before AVBOIT accumulation evaluates surface inputs, CSG and the authored material hook. That can avoid work for the discarded fragment, but it introduces earlier termination of neighboring fragment invocations whose values can still be needed for derivatives. The primary-instance ID comes from a texture and can vary within a primitive or a 2Ã—2 derivative group. The existing device and material contracts do not guarantee that this new rejection is safe.

The current CSG path makes the dependency concrete: `nwbAvboitCsgPixelDiscard` calls `nwbCsgPixelWorldDistanceTolerance`, which evaluates `ddx` and `ddy` of world position. Authored `nwbMaterialSurface` hooks also have no derivative-free restriction and may use implicit-LOD texture sampling. Evaluating only the CSG footprint before the early rejection therefore would not establish safety for the subsequent material hook.

The preserved emitted SPIR-V provides supporting evidence from the existing shared-material path. Its CSG module contains five `OpKill` instructions, one `OpDPdx`, one `OpDPdy` and one implicit-LOD sample; the ordinary module contains four `OpKill` instructions and one implicit-LOD sample. Neither contains `OpDemoteToHelperInvocation`. These are observations of the saved current-path modules, not results from an implemented early-rejection candidate. The five source identities in the proof manifest still match the reviewed production files, including the fragment path, CSG helper, shader cooker and Vulkan capability contract.

`OpKill` does not provide a portable promise that the invocation will continue as a helper for subsequent derivative work. The current capability/compiler baseline also does not require shader quad control, maximal reconvergence or helper-inclusive quad voting. An ordinary subgroup ballot or an assumed quad-uniform predicate would not supply the missing guarantee. The relevant primary references are the [Vulkan derivative-operation requirements](https://docs.vulkan.org/spec/latest/chapters/shaders.html#shaders-derivative-operations) and the [Khronos shader quad-control proposal](https://docs.vulkan.org/features/latest/features/proposals/VK_KHR_shader_quad_control.html). Opcode decoding in the saved evidence uses the [Khronos SPIR-V grammar](https://raw.githubusercontent.com/KhronosGroup/SPIRV-Headers/main/include/spirv/unified1/spirv.core.grammar.json).
...
## Step 10: known-air reflection specialization evaluated and rejected

The implemented experiment compiled a smaller optical reflection entry for origins proven outside the complete conservative transparent bounds. Variant 3 handled those rays first, wrote their results/counters, and retired their admitted queue words with `0xffffffff`. A forced queue UAV dependency then preceded full variant 2, which skipped the marker before receiver decoding and performed ordinary bootstrap for remaining entries. Both dispatches shared the original admission prefix, ray budget, indirect arguments, frame parameters, hardware task and timing range. Queue words remained four bytes and frame constants remained 192 bytes. The graph declared the queue read/write only for this route. Complete scenes without transparent instances retained plain variant 0.

Three exact states separated factoring from partitioning: A was original production at `eb3ab49df`; B used the shared continuation with full reference variant 1; C selected air 3 followed by full 2. The entire B/C source difference was one private compile-time selector in `reflection_system.cpp`. Their generated inputs, shader/toolchain artifacts, authored runtime and dependencies were identical; only the executable changed. Both states used the same repaired fifteen-path source lineage. The original candidate packages remain unchanged: the native fixture needed an explicit `DivideUp<u32>` template argument to compile, and the added `.nwb` source banner/footer was removed to satisfy `.helper` section 15. The initial build failure and both narrow repairs are preserved.

Optimized/debug candidate builds and ordinary cooks passed. All 382 active ECS graphics cases and both native raytrace upload/resource-reuse cases passed in each configuration. The new production-cooker/native-readback fixture ran one GoogleTest containing **1,191 actual cases per configuration**, without skips. It covered original candidate/budget/capacity limits, tails, pre-retired words, mixed opaque/glass identities, zero-weight and invalid receivers, finite/nonfinite metadata, and face/edge/corner boundaries. Its uninterrupted air-dispatch/barrier/full-dispatch bank matched the reference and staged banks in complete queue, counter and output bytes. The fixture used zero optical queries and a real built empty-instance TLAS: it establishes tested admission/ownership behavior, not positive-query radiometry.

All three actual Opt native modules passed SPIR-V validation and exactly matched their Dbg counterparts, ordinary cooked modules and indexed packed-runtime bytes. The thirty-case positive-query optical suite then ran twice: A to B and B to C. Both stages passed all sixty captures, with actual GPU-validation startup markers, unchanged scene oracles, exact decoded RGB parity and exact history-qualified stable semantic diagnostics. That is 120 captures across two comparisons of the same thirty cases. The original continuation source substring remained unchanged.

The emitted review confirmed the sentinel-before-decode path, conservative finite expanded bounds, use of the same proven origin value by the initial ray query, absence of the bootstrap collector from the air entry, and result/counter completion before retirement. The variant-2 structural witness passed. The variant-3 checker explicitly rejected its narrow requirement for identical pointer IDs: the compiler recomputed the same admitted queue address. The retained manual def-use/CFG review establishes identical immutable selectors and integer offsets and checks the remaining output/retirement conditions. No helper or shader was changed to force that check to pass. The modules emit no `NoContraction` decoration; the review relies on the same carried value and conservative expansion under the stated conforming FP32/metadata assumptions, not cross-kernel rounding equality. All 39 offline emitted-helper tests passed. The host review separately traced the forced same-state dependency through shader read/write access and compute stages to `vkCmdPipelineBarrier2`, including readiness and zero-hardware gates.

Optimized qualification also passed ten regression targets: ECS graphics, native raytrace, depth/spatial kernels, ordinary/rough/temporal reflection, the full refraction gallery, duplicate comparisons, mid-pose transparent CSG and combined caustics/refraction/reflection. Debug CPU/native regressions and hardware-required refraction and combined-optics captures with GPU validation passed. The combined image was inspected. These domain checks do not claim byte parity for stochastic caustics. Positive-query air scenes have opaque receivers; the suite does not clearly mix air-handled and inside-bootstrap positive-query receivers in one frame or exercise air-retired primary glass. The mixed native fixture covers ownership for those combinations with zero queries.

Before acquisition, the retention protocol required a practical clear-scene full-frame reduction with equivalent controls and an inside-scene upper interval within the existing positive practical tolerance. Only a qualifying B-to-C result would trigger fresh A-to-C clear and inside campaigns; this additional net gate prevents a gain relative to a potentially slower factoring reference from being mistaken for a production gain. No A-to-B timing subtraction or reuse of old A trials was allowed.

Both predeclared B-to-C campaigns completed: 32 launches in sixteen balanced pairs, with 3,619 completed GPU frames and 3,619 hardware ranges. Clear supplied 1,958 frames and inside 1,661. Each campaign used eight blocks, seeds zero, two warm-up intervals, six retained intervals, at least 100 completed frames per launch, a 90-second bound and the unchanged 3%/0.02 ms practical gate. Hardware was required; temporal/spatial/feedback, diagnostics, capture and validation were disabled for timing. Resolution was 960x720, the ray budget 1,382,400 and optical-query limit 16. One range contained one B dispatch or both C dispatches and their barrier; duration was not divided by two.

| Workload / scope | B / C mean ms | Paired C-minus-B delta ms [95% interval] |
| --- | ---: | --- |
| Clear / full GPU frame | 48.677271 / 22.405305 | -26.271966 [-26.948599, -25.853539] |
| Clear / hardware reflection | 33.624523 / 4.202408 | -29.422115 [-30.048070, -29.008322] |
| Inside / full GPU frame | 69.196464 / 73.264647 | +4.068183 [-1.958640, +9.749638] |
| Inside / hardware reflection | 53.665508 / 53.988962 | +0.323455 [-5.600576, +5.880717] |

The clear-scene reduction is a strong measured observation in every pair, not a claim of no benefit. However, the runner reports `control_drift` for both workloads. Clear opaque timing shifted by -0.074614 ms, interval [-0.104205, -0.047573], outside its 0.027888 ms tolerance; shadow shifted by +3.493092 ms, interval [+3.381965, +3.590691], outside 0.264653 ms. Lighting equivalence remained uncertain. Inside shadow shifted by +3.771647 ms, interval [+3.696755, +3.849284], outside 0.274735 ms; opaque and lighting controls remained uncertain. Shadow was slower in every pair of both campaigns. Its cause is not established by source/asset isolation, and these changes are retained rather than attributed to an invented bottleneck.

Inside also fails the explicit noninferiority gate: its full-frame interval upper bound is 9.749638 ms versus the permitted 2.075894 ms. The interval crosses zero, so this is inability to exclude a material slowdown, not proof of a causal full-frame regression. Hardware excursions occurred in opposing directions in inside blocks 1 and 7; neither was removed. No trial was discarded, extended or selectively repeated. Measurements used the Qualcomm Adreno X2-90 GPU with the recorded mesh-compute-emulation route. Balanced power and 78% battery were reported at both endpoints; frequency and thermal telemetry were unavailable. No build, cook or other GPU acquisition overlapped timing.

The experiment is rejected under the declared controls and inside-overhead gates despite the promising clear-path observation. Conditional A-to-C net campaigns and the prepared selector-removal cleanup were not executed; no net production speedup is claimed. All nine existing files were restored to exact before hashes and all six candidate-only files were removed. No experimental pipeline selector, queue protocol or native target remains in production. Candidate source, modules, images, failed/unsupported checks and every timing trial remain in the local evidence.

Restored Opt/Dbg builds and ordinary 206-asset cooks passed, followed by all 381 active ECS graphics cases, both native raytrace cases, and both depth/spatial GoogleTests containing 39 depth and 160 spatial scenarios. The complete restored Opt authored runtime and dependencies match A. An initial strict executable-hash comparison correctly failed: the rebuilt executable differs only in four bytes within the COFF and debug-directory linker timestamps; every other byte matches A. The timestamp-only proof is retained separately from the failed exact-hash check. Final restored hardware-required Opt combined-optics and Dbg refraction captures passed with GPU validation; the Opt combined logs confirm the enabled Vulkan validation layer.

Evidence is under `__artifacts/reflection_optimization_steps/step10/`: `source_lineage_v3.json`, the A/B/C freezes, native GTest XML and module snapshots, `native_cooked_packed_linkage_v1.json`, `inventory_opt_*`, `witness_opt_*`, `emitted_review_rt`, `host_review`, both optical parity stages, candidate regression logs, `retention_timing_protocol.json`, both timing campaigns and power records, and restoration source/build/test/identity/visual records. Source-lineage SHA256 is `fcbf53001a9bfee720182510c40665f0ee0f04ec211d4d7abeffbe3e6a0ab3b3`. Reproduction follows the retained sequential execution guide with corrected native V2 and the two recorded repairs; native tests use `nwb_reflection_optical_retirement_kernel_tests`, optical comparisons use `step10-factor` then predecessor-bound `step10-partition`, and timing uses the common runner with `reflection-optical-clear`/`reflection-optical-inside` and explicit dispatch annotations 1/2.

Factoring parity SHA256: `0a4573a1c08a68060d9475c5a6f0e934c6e670a692e777078b6b335e989e3359`.

Partition parity SHA256: `96c5169ab0e07251325059b4e769429801e285e595a4dc003534ca79e3c35f9d`.
