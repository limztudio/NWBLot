...
namespace SkinnedGeometryMorphPayload{
...
[[nodiscard]] inline bool BuildResolvedMorphWeightLookup(const SkinnedGeometryRuntimeMeshInstance& instance, const SkinnedGeometryMorphWeightsComponent* weights, MorphWeightLookup& outWeights){
    Name failedMorph = NAME_NONE;
    if(SkinnedGeometryRuntime::BuildMorphWeightSumLookup(instance.morphs, weights, outWeights, failedMorph))
        return true;

    NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometrySystem: runtime mesh '{}' morph '{}' weight is invalid")
...
[[nodiscard]] inline bool ActiveDeltaVector(const SIMDVector value, const u32 activeMask){
    ...
    const SIMDVector active = VectorGreater(VectorAbs(value), VectorReplicate(SkinnedGeometryValidation::s_Epsilon));
...
[[nodiscard]] inline MorphDeltaVectors LoadMorphDelta(const SkinnedGeometryMorphDelta& delta){
...
[[nodiscard]] inline bool ValidMorphDelta(const SkinnedGeometryMorphDelta& delta, const MorphDeltaVectors& deltaVectors, const usize vertexCount){
    return
        delta.vertexId < vertexCount
        && SkinnedGeometryValidation::FiniteVector(deltaVectors.deltaPosition, 0x7u)
        && SkinnedGeometryValidation::FiniteVector(deltaVectors.deltaNormal, 0x7u)
        && SkinnedGeometryValidation::FiniteVector(deltaVectors.deltaTangent, 0xFu)
    ;
}
...
[[nodiscard]] inline bool ValidateRuntimeMeshVertexCount(const SkinnedGeometryRuntimeMeshInstance& instance){
    if(instance.restVertices.size() <= static_cast<usize>(Limit<u32>::s_Max))
        return true;

    NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometrySystem: runtime mesh '{}' vertex count exceeds u32 limits"), instance.handle.value);
    return false;
}
...
[[nodiscard]] bool BuildBlendedMorphPayload(
    ...
    for(const SkinnedGeometryMorph& morph : instance.morphs){
        const f32 weight = ResolvedMorphWeight(resolvedWeights, morph.name);
        if(!SkinnedGeometryValidation::ActiveWeight(weight))
            continue;
        if(morph.deltas.empty()){
            NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometrySystem: active morph '{}' on runtime mesh '{}' has no deltas")
        ...
        if(
            ...
            NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometrySystem: morph '{}' on runtime mesh '{}' exceeds u32 delta limits")
                , StringConvert(morph.name.c_str())
                , instance.handle.value
            );
            return false;
        ...
        for(const SkinnedGeometryMorphDelta& delta : morph.deltas){
            const MorphDeltaVectors deltaVectors = LoadMorphDelta(delta);
            if(!ValidMorphDelta(delta, deltaVectors, vertexCount)){
                NWB_LOGGER_ERROR(NWB_TEXT("SkinnedGeometrySystem: morph '{}' on runtime mesh '{}' contains an invalid delta")
                    , StringConvert(morph.name.c_str())
                    , instance.handle.value
                );
                return false;
    ...
    for(const u32 vertexIndex : touchedVertices){
        ...
        SkinnedGeometryVertexMorphRangeGpu range;
        range.firstDelta = static_cast<u32>(outDeltas.size());
        range.deltaCount = 1u;
        outRanges[vertexIndex] = range;

        SkinnedGeometryBlendedMorphDeltaGpu gpuDelta;
