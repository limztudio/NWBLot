...
enum tlsf_private
{
    ...
    */
    ...
    FL_INDEX_SHIFT = (static_cast<int>(SL_INDEX_COUNT_LOG2) + static_cast<int>(ALIGN_SIZE_LOG2)),
...
static constexpr const size_t block_size_max = static_cast<size_t>(1) << FL_INDEX_MAX;
...
static block_header_t* block_from_ptr(const void* ptr){
    return reinterpret_cast<block_header_t*>(
        const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(ptr)) - block_start_offset);
}

static void* block_to_ptr(const block_header_t* block){
    return const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(block)) + block_start_offset;
}

/* Return location of next block after block of given size. */
static block_header_t* offset_to_block(const void* ptr, size_t size){
    return reinterpret_cast<block_header_t*>(reinterpret_cast<tlsfptr_t>(ptr) + size);
}
...
static void integrity_walker(void* ptr, size_t size, int used, void* user){
    ...
    integrity_t* integ = static_cast<integrity_t*>(user);
...
int tlsf_check(tlsf_t tlsf){
    ...
    control_t* control = static_cast<control_t*>(tlsf);
    int status = 0;
...
static void default_walker(void* ptr, size_t size, int used, void* user){
    ...
    tlsf_report("\t%p %s size: %x (%p)\n", ptr, used ? "used" : "free", static_cast<unsigned int>(size), block_from_ptr(ptr));
}

void tlsf_walk_pool(pool_t pool, tlsf_walker walker, void* user){
    tlsf_walker pool_walker = walker ? walker : default_walker;
    block_header_t* block =
        offset_to_block(pool, static_cast<size_t>(-static_cast<int>(block_header_overhead)));
...
pool_t tlsf_add_pool(tlsf_t tlsf, void* mem, size_t bytes){
    ...
    */
    block = offset_to_block(mem, static_cast<size_t>(-static_cast<tlsfptr_t>(block_header_overhead)));
    block_set_size(block, pool_bytes);
    block_set_free(block);
    block_set_prev_used(block);
    block_insert(static_cast<control_t*>(tlsf), block);
...
void tlsf_remove_pool(tlsf_t tlsf, pool_t pool){
    control_t* control = static_cast<control_t*>(tlsf);
    block_header_t* block = offset_to_block(pool, static_cast<size_t>(-static_cast<int>(block_header_overhead)));
...
tlsf_t tlsf_create(void* mem){
    ...
    control_construct(static_cast<control_t*>(mem));
...
void tlsf_destroy(tlsf_t tlsf){
    /* Nothing to do. */
    static_cast<void>(tlsf);
}

pool_t tlsf_get_pool(tlsf_t tlsf){
    return static_cast<pool_t>(static_cast<char*>(tlsf) + tlsf_size());
}
