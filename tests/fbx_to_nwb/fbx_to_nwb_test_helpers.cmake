...
function(require_crlf_text_file path label)
    # file(READ) normalizes line endings in a CMake string on some hosts, so inspect raw bytes instead.
    file(READ "${path}" text_hex HEX)
    string(FIND "${text_hex}" "0d0a" crlf_offset)
    if(crlf_offset EQUAL -1)
        message(FATAL_ERROR "${label} did not contain CRLF line endings")
    endif()

    string(LENGTH "${text_hex}" text_hex_length)
    math(EXPR last_byte_offset "${text_hex_length} - 2")
    foreach(byte_offset RANGE 0 ${last_byte_offset} 2)
        string(SUBSTRING "${text_hex}" ${byte_offset} 2 byte)
        if(byte STREQUAL "0a")
            math(EXPR previous_byte_offset "${byte_offset} - 2")
            if(previous_byte_offset LESS 0)
                message(FATAL_ERROR "${label} began with a bare LF")
            endif()

            string(SUBSTRING "${text_hex}" ${previous_byte_offset} 2 previous_byte)
            if(NOT previous_byte STREQUAL "0d")
                message(FATAL_ERROR "${label} contained a bare LF")
            endif()
        endif()
    endforeach()
endfunction()
