...
- The normal deferred Shadow Visibility task now declares its descriptor-visible hardware and software traversal
  ...
  entry transitions. Direct or minimal callers retain native setup by default; the preflight-failure `CopyDest` clear,
  opaque-to-transparent image/UAV boundary, and later intra-task soft/hybrid UAV fences remain native ownership.
