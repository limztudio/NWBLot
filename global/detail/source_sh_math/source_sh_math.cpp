...
#include "source_sh_math.h"

#include "../../assert.h"


#if NWB_COMPILER_MSVC
#pragma warning(disable : 4619 4456 5264)
#endif

#if NWB_COMPILER_CLANG
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
namespace __hidden_source_sh_math{

using namespace SourceMathInternal;
using namespace SourceShMathInternal;

#ifdef _PREFAST_
#pragma prefast(disable:246, "generated code by maple (nested const variable definitions)")
#endif

    const float fExtraNormFac[SH_MAX_ORDER] = { 2.0f*sqrtf(MATH_PI), 2.0f / 3.0f*sqrtf(3.0f*MATH_PI), 2.0f / 5.0f*sqrtf(5.0f*MATH_PI), 2.0f / 7.0f*sqrtf(7.0f*MATH_PI), 2.0f / 3.0f*sqrtf(MATH_PI), 2.0f / 11.0f*sqrtf(11.0f*MATH_PI) };
    ...
    NWB_INLINE void ComputeCapInt(const size_t order, float angle, float* pR)
    {
    ...
    // lights.
    NWB_INLINE float CosWtInt(const size_t order)
    {
    ...
    const float SHEvalHemisphereLight_fSqrtPi = sqrtf(MATH_PI);
    const float SHEvalHemisphereLight_fSqrtPi3 = sqrtf(MATH_PI / 3.0f);
...
#define CONSTANT(x) (x ## f)
    ...
    //
    NWB_INLINE void sh_eval_basis_1(REAL x, REAL y, REAL z, REAL b[4])
    {
        /* m=0 */
    ...
    // output is vector b with SH basis evaluated at (x,y,z).
    //
    NWB_INLINE void sh_eval_basis_2(REAL x, REAL y, REAL z, REAL b[9])
    {
        const REAL z2 = z*z;
    ...
    NWB_INLINE void rot(REAL ct, REAL st, REAL x, REAL y, REAL &xout, REAL &yout)
    {
        xout = x*ct - y*st;
        yout = y*ct + x*st;
    }

    NWB_INLINE void rot_inv(REAL ct, REAL st, REAL x, REAL y, REAL &xout, REAL &yout)
    {
        xout = x*ct + y*st;
        yout = y*ct - x*st;
    }

    NWB_INLINE void rot_1(REAL ct, REAL st, REAL ctm[1], REAL stm[1])
    {
        ctm[0] = ct;
        stm[0] = st;
    }

    NWB_INLINE void rot_2(REAL ct, REAL st, REAL ctm[2], REAL stm[2])
    {
    ...
    NWB_INLINE void rot_3(REAL ct, REAL st, REAL ctm[3], REAL stm[3])
    {
        REAL ct2 = CONSTANT(2.0)*ct;
        ctm[0] = ct;
        stm[0] = st;
    ...
    NWB_INLINE void rot_4(REAL ct, REAL st, REAL ctm[4], REAL stm[4])
    {
        REAL ct2 = CONSTANT(2.0)*ct;
        ctm[0] = ct;
        stm[0] = st;
    ...
    NWB_INLINE void rot_5(REAL ct, REAL st, REAL ctm[5], REAL stm[5])
    {
        REAL ct2 = CONSTANT(2.0)*ct;
        ctm[0] = ct;
        stm[0] = st;
    ...
    NWB_INLINE void sh_rotz_1(REAL ctm[1], REAL stm[1], REAL y[NL1], REAL yr[NL1])
    {
        yr[1] = y[1];
        rot_inv(ctm[0], stm[0], y[0], y[2], yr[0], yr[2]);
    }

    NWB_INLINE void sh_rotz_2(REAL ctm[2], REAL stm[2], REAL y[NL2], REAL yr[NL2])
    {
        yr[2] = y[2];
    ...
    NWB_INLINE void sh_rotz_3(REAL ctm[3], REAL stm[3], REAL y[NL3], REAL yr[NL3])
    {
        yr[3] = y[3];
    ...
    NWB_INLINE void sh_rotz_4(REAL ctm[4], REAL stm[4], REAL y[NL4], REAL yr[NL4])
    {
        yr[4] = y[4];
    ...
    NWB_INLINE void sh_rotz_5(REAL ctm[5], REAL stm[5], REAL y[NL5], REAL yr[NL5])
    {
        yr[5] = y[5];
    ...
    NWB_INLINE void sh_rotx90_1(REAL y[], REAL yr[])
    {
        yr[0] = fx_1_001*y[1];
        yr[1] = fx_1_002*y[0];
        yr[2] = fx_1_001*y[2];
    ...
    NWB_INLINE void sh_rotx90_inv_1(REAL y[], REAL yr[])
    {
        yr[0] = fx_1_002*y[1];
        yr[1] = fx_1_001*y[0];
        yr[2] = fx_1_001*y[2];
    ...
    NWB_INLINE void sh_rotx90_2(REAL y[], REAL yr[])
    {
        yr[0] = fx_2_001*y[3];
        yr[1] = fx_2_002*y[1];
    ...
    NWB_INLINE void sh_rotx90_inv_2(REAL y[], REAL yr[])
    {
        yr[0] = fx_2_002*y[3];
        yr[1] = fx_2_002*y[1];
        yr[2] = fx_2_003*y[2] + fx_2_004*y[4];
    ...
    NWB_INLINE void sh_rotx90_3(REAL y[], REAL yr[])
    {
    ...
    NWB_INLINE void sh_rotx90_inv_3(REAL y[], REAL yr[])
    {
    ...
    NWB_INLINE void sh_rotx90_4(REAL y[], REAL yr[])
    {
    ...
    NWB_INLINE void sh_rotx90_inv_4(REAL y[], REAL yr[])
    {
    ...
    NWB_INLINE void sh_rotx90_5(REAL y[], REAL yr[])
    {
    ...
    NWB_INLINE void sh_rotx90_inv_5(REAL y[], REAL yr[])
    {
    ...
    NWB_INLINE void sh_rot_1(REAL m[3 * 3], REAL y[NL1], REAL yr[NL1])
    {
    ...
    NWB_INLINE void sh_roty_1(REAL ctm[1], REAL stm[1], REAL y[NL1], REAL yr[NL1])
    {
        yr[0] = y[0];
        rot_inv(ctm[0], stm[0], y[1], y[2], yr[1], yr[2]);
    }

    NWB_INLINE void sh_roty_2(REAL ctm[2], REAL stm[2], REAL y[NL2], REAL yr[NL2])
    {
        REAL ytmp[NL2];
        sh_rotx90_2(y, yr);
    ...
    NWB_INLINE void sh_roty_3(REAL ctm[3], REAL stm[3], REAL y[NL3], REAL yr[NL3])
    {
        REAL ytmp[NL3];
        sh_rotx90_3(y, yr);
    ...
    NWB_INLINE void sh_roty_4(REAL ctm[4], REAL stm[4], REAL y[NL4], REAL yr[NL4])
    {
        REAL ytmp[NL4];
        sh_rotx90_4(y, yr);
    ...
    NWB_INLINE void sh_roty_5(REAL ctm[5], REAL stm[5], REAL y[NL5], REAL yr[NL5])
    {
        REAL ytmp[NL5];
        sh_rotx90_5(y, yr);
...
#define ROT_TOL CONSTANT(1e-4)
    ...
    */
    NWB_INLINE void zyz(REAL m[3 * 3], REAL &zc1, REAL &zs1, REAL &yc, REAL &ys, REAL &zc2, REAL &zs2)
    {
        REAL cz = m[8];
    ...
    NWB_INLINE void sh_rotzyz_2(REAL zc1m[2], REAL zs1m[2], REAL ycm[2], REAL ysm[2], REAL zc2m[2], REAL zs2m[2], REAL y[NL2], REAL yr[NL2])
    {
        REAL ytmp[NL2];
    ...
    NWB_INLINE void sh_rotzyz_3(REAL zc1m[3], REAL zs1m[3], REAL ycm[3], REAL ysm[3], REAL zc2m[3], REAL zs2m[3], REAL y[NL3], REAL yr[NL3])
    {
        REAL ytmp[NL3];
    ...
    NWB_INLINE void sh_rotzyz_4(REAL zc1m[4], REAL zs1m[4], REAL ycm[4], REAL ysm[4], REAL zc2m[4], REAL zs2m[4], REAL y[NL4], REAL yr[NL4])
    {
        REAL ytmp[NL4];
    ...
    NWB_INLINE void sh_rotzyz_5(REAL zc1m[5], REAL zs1m[5], REAL ycm[5], REAL ysm[5], REAL zc2m[5], REAL zs2m[5], REAL y[NL5], REAL yr[NL5])
    {
        REAL ytmp[NL5];
    ...
    NWB_INLINE void sh3_rot(REAL m[3 * 3], REAL zc1, REAL zs1, REAL yc, REAL ys, REAL zc2, REAL zs2, REAL y[NSH3], REAL yr[NSH3])
    {
        REAL zc1m[3], zs1m[3];
    ...
    NWB_INLINE void sh4_rot(REAL m[3 * 3], REAL zc1, REAL zs1, REAL yc, REAL ys, REAL zc2, REAL zs2, REAL y[NSH4], REAL yr[NSH4])
    {
        REAL zc1m[4], zs1m[4];
    ...
    NWB_INLINE void sh5_rot(REAL m[3 * 3], REAL zc1, REAL zs1, REAL yc, REAL ys, REAL zc2, REAL zs2, REAL y[NSH5], REAL yr[NSH5])
    {
        REAL zc1m[5], zs1m[5];
    ...
    NWB_INLINE void sh1_rot(REAL m[3 * 3], REAL y[NSH1], REAL yr[NSH1])
    {
        yr[0] = y[0];
        sh_rot_1(m, y + NSH0, yr + NSH0);
    }

    NWB_INLINE void sh3_rot(REAL m[3 * 3], REAL y[NSH3], REAL yr[NSH3])
    {
    ...
    NWB_INLINE void sh4_rot(REAL m[3 * 3], REAL y[NSH4], REAL yr[NSH4])
    {
        REAL zc1, zs1, yc, ys, zc2, zs2;
        zyz(m, zc1, zs1, yc, ys, zc2, zs2);
    ...
    NWB_INLINE void sh5_rot(REAL m[3 * 3], REAL y[NSH5], REAL yr[NSH5])
    {
        REAL zc1, zs1, yc, ys, zc2, zs2;
        zyz(m, zc1, zs1, yc, ys, zc2, zs2);
    ...
    NWB_INLINE void SimpMatMul(size_t dim, const float* matrix, const float* input, float* result)
    {
...
using namespace SourceMathInternal;
using namespace __hidden_source_sh_math;
...
//-------------------------------------------------------------------------------------
float* MathCallConv SHEvalDirection(
    float* result,
    size_t order,
    VectorArg dir)noexcept
{
    if (!result)
        return nullptr;
    ...
    default:
        NWB_ASSERT(order < SH_MIN_ORDER || order > SH_MAX_ORDER);
        return nullptr;
    }
    ...
    const float* input)noexcept
{
    ...
    Float4 column1;
    Float4 column2;
    StoreFloat4(&column0, rotMatrix.r[0]);
    StoreFloat4(&column1, rotMatrix.r[1]);
    StoreFloat4(&column2, rotMatrix.r[2]);

    // SH coefficient rotation uses the inverse spatial basis rotation. For
    // orthonormal rotation matrices under NWB's column-vector convention, that
    // inverse is the transpose. Instead of materializing a transposed matrix at
    // the callsite, extract the needed transposed 3x3 basis directly from the
    // original matrix columns here.
    float mRot[3 * 3];
    const float r00 = mRot[0 * 3 + 0] = column0.x;
    const float r10 = mRot[1 * 3 + 0] = column0.y;
    const float r20 = mRot[2 * 3 + 0] = column0.z;

    const float r01 = mRot[0 * 3 + 1] = column1.x;
    const float r11 = mRot[1 * 3 + 1] = column1.y;
    const float r21 = mRot[2 * 3 + 1] = column1.z;

    const float r02 = mRot[0 * 3 + 2] = column2.x;
    const float r12 = mRot[1 * 3 + 2] = column2.y;
    const float r22 = mRot[2 * 3 + 2] = column2.z;
    ...
    const float* input)noexcept
{
    ...
    if (order < SH_MIN_ORDER || order > SH_MAX_ORDER)
        return nullptr;

    float R[(2 * (SH_MAX_ORDER - 1) + 1)*(2 * (SH_MAX_ORDER - 1) + 1)]; // used to store rotation matrices...
    ...
    const float* inputG)noexcept
{
    ...
    {
    case 2:
        return SHMultiply2(result, inputF, inputG);

    case 3:
        return SHMultiply3(result, inputF, inputG);

    case 4:
        return SHMultiply4(result, inputF, inputG);

    case 5:
        return SHMultiply5(result, inputF, inputG);

    case 6:
        return SHMultiply6(result, inputF, inputG);
    ...
    float* resultB)noexcept
{
    ...
    AlignedFloat3 clr;
    StoreFloat3A(&clr, color);

    float fTmp[SH_MAX_ORDER * SH_MAX_ORDER];

    SHEvalDirection(fTmp, order, dir); // evaluate the BF in this direction...

    // now compute "normalization" and scale vector for each valid spectral band
    const float fNorm = MATH_PI / CosWtInt(order);
    ...
    float* resultB)noexcept
{
    ...
    const float fDist = VectorGetX(Vector3Length(pos));
    ...
    const float fConeAngle = (fDist <= radius) ? (MATH_PIDIV2) : asinf(radius / fDist);

    Vector dir = Vector3Normalize(pos);

    float fTmpDir[SH_MAX_ORDER* SH_MAX_ORDER];  // rotation "vector"
    float fTmpL0[SH_MAX_ORDER];
    ...
    float* resultB)noexcept
{
    ...
    if (radius < 0.f || radius >(MATH_PI*1.00001f))
        return false;

    if (radius < 0.0001f)
    {
        // turn it into a pure directional light...
        return SHEvalDirectionalLight(order, dir, color, resultR, resultG, resultB);
    }
    else
    {
        float fTmpL0[SH_MAX_ORDER];
        float fTmpDir[SH_MAX_ORDER * SH_MAX_ORDER];

        const float fConeAngle = radius;
        const float fAngCheck = (fConeAngle > MATH_PIDIV2) ? (MATH_PIDIV2) : fConeAngle;
    ...
    float* resultB)noexcept
{
    ...
    AlignedFloat3 clrTop;
    StoreFloat3A(&clrTop, topColor);

    AlignedFloat3 clrBottom;
    StoreFloat3A(&clrBottom, bottomColor);
