...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  irradiance clear at the front of the same caustics packet. Direct callers retain native setup by default, while
  temporal accumulator reset/decay and the multi-pass resolve/UAV sequence remain native task-local ownership.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  owns the unconditional black irradiance clear at the front of the same caustics packet. Direct callers retain
  native setup by default; temporal accumulation and the multi-pass resolve/ping-pong transitions remain native
