...
static void TestSurfaceEditOperatorRemeshStaysOnSideHitPatch(TestContext& context){
    ...
    const usize oldVertexCount = instance.restVertices.size();
    NWB::Impl::DeformableHoleEditParams params =
        MakeHoleEditParams(instance, 5u, 0.10f, 0.10f, 0.80f, 0.55f, 0.60f)
    ;
    params.operatorFootprint = MakeBoxOperatorFootprint();
    params.operatorProfile = MakeBoxOperatorProfile();
    ...
    bool foundTopFaceRim = false;
    for(usize vertexIndex = oldVertexCount; vertexIndex < static_cast<usize>(result.firstWallVertex); ++vertexIndex){
        const Float3U& position = instance.restVertices[vertexIndex].position;
        foundRightFaceRim = foundRightFaceRim
            || (
        ...
        ;
        foundBackFaceRim = foundBackFaceRim
            || (
                NearlyEqual(position.y, 1.0f, 0.0001f)
                && position.x < 1.0f - 0.0001f
                && position.x > 1.0f - params.depth - 0.0001f
            )
        ;
        foundTopFaceRim = foundTopFaceRim
            || (
                NearlyEqual(position.z, 0.0f, 0.0001f)
                && position.x < 1.0f - 0.0001f
                && position.x > 1.0f - params.depth - 0.0001f
            )
        ;
...
static void TestSurfaceEditFlowAttachesAndPersistsAccessory(TestContext& context){
    ...
    NWB::Impl::TransformComponent baseTransform;
    instance.dirtyFlags = static_cast<NWB::Impl::RuntimeMeshDirtyFlags>(
        instance.dirtyFlags | NWB::Impl::RuntimeMeshDirtyFlag::GpuUploadDirty
    );
    NWB_ECS_GRAPHICS_TEST_CHECK(
    ...
    NWB::Impl::TransformComponent localTransform;
    NWB_ECS_GRAPHICS_TEST_CHECK(
        context,
        NWB::Impl::ResolveAccessoryAttachmentTransform(
            instance,
    ...
    NWB::Impl::TransformComponent translatedTransform;
    NWB_ECS_GRAPHICS_TEST_CHECK(
        context,
        NWB::Impl::ResolveAccessoryAttachmentTransform(
            instance,
...
static void TestSurfaceEditStateReplayRestoresAccessory(TestContext& context){
    ...
    NWB::Impl::DeformableAccessoryAttachmentComponent restoredAttachment;
    testWorld.world.view<
        NWB::Impl::GeometryComponent,
        NWB::Impl::RendererComponent,
        NWB::Impl::DeformableAccessoryAttachmentComponent
    >().each(
        [&](NWB::Core::ECS::EntityID,
            NWB::Impl::GeometryComponent& geometry,
            NWB::Impl::RendererComponent& renderer,
            NWB::Impl::DeformableAccessoryAttachmentComponent& restored){
            ++accessoryCount;
            restoredAttachment = restored;
            NWB_ECS_GRAPHICS_TEST_CHECK(context, !renderer.visible);
            NWB_ECS_GRAPHICS_TEST_CHECK(context, geometry.geometry.name() == mockGeometry);
...
static void TestSurfaceEditStateReplayRestoresMultipleAccessories(TestContext& context){
    ...
    NWB::Impl::TransformComponent oldHoleTransform;
    NWB_ECS_GRAPHICS_TEST_CHECK(
        context,
        NWB::Impl::ResolveAccessoryAttachmentTransform(
            editedInstance,
...
static void TestSurfaceEditRedoLastReplaysFromCleanBase(TestContext& context){
    ...
    NWB::Impl::TransformComponent expectedSecondTransform;
    NWB_ECS_GRAPHICS_TEST_CHECK(
        context,
        NWB::Impl::ResolveAccessoryAttachmentTransform(
            editedInstance,
