...
// Three opaque compute-emulation items can reuse one persistent generated-vertex buffer only when every dispatch
// is immediately followed by its raster consumer. The expanded bounded path must keep the same graph-owned
// alternation as the pair: D(A) -> R(A) -> D(B) -> R(B) -> D(C) -> R(C).
...
// AVBOIT extinction can prepare several transparent compute-emulated materials before its raster stage only when
// each generated-vertex target is distinct. Keep that alias-free producer/consumer handoff graph-owned: the
// Graphics|Compute producer writes every target as UAV, then the immediate Extinction raster reads them as vertex
...
// Occupancy is the first raster phase on the split AVBOIT route. Its graph-owned generator must follow the final
// target clear, remain adjacent to raster in AVBOIT Pre's Graphics packet, and publish coverage for the following
// dedicated-Compute depth warp without introducing another packet.
