...
// that will be provided to user by __NWB_alloc_replacement_log()
namespace Log{
    ...
    static void record(FunctionInfo functionInfo, const char* opcodeString, bool status){
        assert(functionInfo.dllName && "Empty DLL name value");
        assert(functionInfo.funcName && "Empty function name value");
        assert(opcodeString && "Empty opcode");
        assert((record_number <= RECORDS_COUNT) && "Incorrect record number");
        ...
        assert(entry && "Invalid memory was returned");

#ifdef NWB_UNICODE
        auto dllNameObj = convert(reinterpret_cast<const wchar_t*>(functionInfo.dllName));
        const auto* dllName = dllNameObj.c_str();
#else
        const auto* dllName = reinterpret_cast<const char*>(functionInfo.dllName);
#endif
        LOG_PRINT(entry, RECORD_LENGTH, "%s: %s (%s), byte pattern: <%s>", status ? "Success" : "Fail", functionInfo.funcName, dllName, opcodeString);
...
size_t compareStrings(const char* str1, const char* str2){
    for(size_t i = 0; str1[i] != 0; i++){
        if(str1[i] != '*' && str1[i] != '#' && str1[i] != str2[i])
            return 0;
    }
    return 1;
}
...
static UINT CheckOpcodes(const char** opcodes, void* inpAddr, bool abortOnError, const FunctionInfo* functionInfo = nullptr){
    ...
    if(!opcodesStringsCount || opcodes_pointer != (size_t)opcodes){
        ...
        assert((maxOpcodesLength / 2 <= MAX_PATTERN_SIZE) && "Pattern exceeded the limit of 28 opcodes/56 symbols");
    }

    // Translate prologue opcodes to string format to compare
    for(i = 0; i < maxOpcodesLength / 2 && i < MAX_PATTERN_SIZE; ++i){
        NWB_SPRINTF(opcodeString + 2 * i, 2 * (MAX_PATTERN_SIZE - i) + 1, "%.2X", *((unsigned char*)inpAddr + i));
    }
    opcodeString[2 * i] = 0;
    ...
    if(abortOnError){
        ...
        assert(false && "CheckOpcodes failed");
    }
    return 0;
}
...
static void CorrectOffset(UINT_PTR address, const char* pattern, UINT distance){
    ...
    if(pos){
        ...
        NWB_MEMCPY(&value, sizeof(value), Addrint2Ptr(address), sizeof(value));
        value += distance;
        NWB_MEMCPY(Addrint2Ptr(address), sizeof(value), &value, sizeof(value));
    }
}
...
static DWORD InsertTrampoline32(void* inpAddr, void* targetAddr, const char* pattern, void** storedAddr){
    ...
    if(storedAddr){ // If requested, store original function code
        ...
        assert((bytesToMove >= SIZE_OF_RELJUMP) && "Incorrect bytecode pattern?");
        ...
        *storedAddr = Addrint2Ptr(trampAddr);
        ...
        NWB_MEMCPY(*storedAddr, bytesToMove, codePtr, bytesToMove);
        ...
        *(UCHAR*)(trampAddr + bytesToMove) = 0xE9;
        NWB_MEMCPY((UCHAR*)(trampAddr + bytesToMove + 1), sizeof(offset32), &offset32, sizeof(offset32));
    }
    ...
    *codePtr = 0xE9;
    NWB_MEMCPY(codePtr + 1, sizeof(offset32), &offset32, sizeof(offset32));
...
static DWORD InsertTrampoline64(void* inpAddr, void* targetAddr, const char* pattern, void** storedAddr){
    ...
    *locPtr = tgtAddr;
    ...
    if(storedAddr){ // If requested, store original function code
        bytesToMove = strlen(pattern) / 2 - 1; // The last byte matching the pattern must not be copied
        assert((bytesToMove >= SIZE_OF_INDJUMP) && "Incorrect bytecode pattern?");
        UINT_PTR trampAddr = memProvider.GetLocation(srcAddr);
        if(!trampAddr)
            return 0;
        *storedAddr = Addrint2Ptr(trampAddr);
    ...
    *(codePtr) = 0xFF;
    *(codePtr + 1) = 0x25;
    NWB_MEMCPY(codePtr + 2, sizeof(offset32), &offset32, sizeof(offset32));
    ...
    if(origFunc){ // Need to store original function code
        ...
        if(*codePtr == 0xE9){ // JMP relative instruction
            ...
            *origFunc = (void*)(Ptr2Addrint(inpAddr) + offsetInJmp + SIZE_OF_RELJUMP);
            ...
            assert((opcodeIdx > 0) && "abortOnError ignored in CheckOpcodes?");
...
FRR_TYPE ReplaceFunctionA(const char* dllName, const char* funcName, FUNCPTR newFunc, const char** opcodes, FUNCPTR* origFunc){
    ...
    if(!cachedHM || strncmp(dllName, cachedName, MAX_PATH) != 0){
        ...
        cachedHM = hModule;
        NWB_STRNCPY(cachedName, MAX_PATH + 1, dllName, MAX_PATH);
    }
...
FRR_TYPE ReplaceFunctionW(const wchar_t* dllName, const char* funcName, FUNCPTR newFunc, const char** opcodes, FUNCPTR* origFunc){
    ...
    if(!cachedHM || wcsncmp(dllName, cachedName, MAX_PATH) != 0){
        ...
        cachedHM = hModule;
        NWB_WSTRNCPY(cachedName, MAX_PATH + 1, dllName, MAX_PATH);
    }
...
// Public Windows API
extern "C" __declspec(dllexport) int __NWB_alloc_replacement_log(char*** function_replacement_log_ptr){
