...
inline constexpr const char* s_GpuCrashReportFileName = "gpu_crash.txt";
inline constexpr const char* s_GpuDetectiveCaptureFileName = "gpu_crash.rgd"; // canonical in-package AMD Radeon GPU Detective capture (decoded server-side through the vendored RGD backend)
inline constexpr const char* s_AftermathGpuDumpFileName = "gpu_crash.nv-gpudmp"; // canonical in-package NVIDIA Nsight Aftermath GPU crash dump (emitted by the device on device-lost, decoded server-side via the Aftermath SDK when present)
