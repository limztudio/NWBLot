...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  `ShaderResource` and resolve-geometry `UnorderedAccess` states without reasserting them natively. Direct callers
  retain native setup by default, while the later accumulator handoff and dynamic resolve-buffer sequence remain
  native task-local ownership. The normal graph also no longer repeats unchanged world/depth sampled-state setup
  across each wavelet pass.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  reasserting them natively. Direct callers retain native setup by default; the later accumulator handoff and
  dynamic resolve/ping-pong transitions remain native task-local ownership. The normal graph also no longer repeats
