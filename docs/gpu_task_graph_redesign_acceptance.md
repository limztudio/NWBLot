...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  irradiance clear and, for a fresh temporal target, a typed accumulator-bootstrap clear at the front of the same
  caustics packet. Once that temporal target is initialized, a route-selected graph-owned decay task declares its
  exact accumulator range as `UnorderedAccess`, starts the existing photon timing interval, and merges immediately
  before the photon producer; the producer receives the compiler-owned same-UAV fence and closes that interval.
  The initialized mirror publishes only after that packet's producer accepts. Direct callers retain native setup by
  default, while non-temporal reset and the multi-pass resolve/UAV sequence remain native task-local ownership.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  owns the unconditional black irradiance clear and, for a fresh temporal target, a typed accumulator-bootstrap
  clear at the front of the same caustics packet. Once that temporal target is initialized, a graph-owned Graphics
  decay task declares the exact accumulator range as `UnorderedAccess`, starts the existing photon timing interval,
  and merges immediately before the hardware photon producer; the producer receives the compiler-owned same-UAV
  fence and closes that interval. The initialized mirror publishes only after that packet's producer accepts.
  Direct callers retain native setup by default; non-temporal reset and the multi-pass resolve/ping-pong transitions
  remain native task-local ownership.
