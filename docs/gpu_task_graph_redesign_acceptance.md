...
- The opaque G-buffer now freezes its material draw ordering and CSG CPU frame payload during graph declaration.
  ...
  opaque native producer, then explicitly hand off from `CopyDest` to `UnorderedAccess`. The receiver-surface
  `csgReceiverEventData` array now joins that pair as an exact graph-declared `UnorderedAccess` image, so normal
  opaque and prepared-transparent material thunks no longer stage either receiver-event image natively; the wider
  CSG target lifecycle and direct compatibility clear remain deliberately outside this bounded task. The transparent CSG
