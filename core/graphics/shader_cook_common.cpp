...
static bool ValidateDefaultVariant(const ManifestEntry& entry, CookArena& arena, AString& outError){
    ...
    if(entry.defaultVariant == "default"){
        ...
        outError = StringFormat(
            "Manifest parsing failed: entry '{}' default variant 'default' is only valid when no entry.define.* values are specified",
            entry.name
        );
        return false;
    }
    ...
    if(entry.defineValues.empty()){
        outError = StringFormat(
            "Manifest parsing failed: entry '{}' default variant '{}' requires entry.define.* values to be specified",
            entry.name,
