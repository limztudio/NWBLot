...
7. Vulkan command buffers and the global descriptor heap must retain every GPU-visible resource/handle until GPU completion; descriptor-buffer ranges are GPU-visible lifetime, not temporary CPU-side state.
...
21. `Core::Filesystem::IFilesystem::writeFileDeferred()` copies payload bytes before returning; caller-local temporary buffers only need to stay alive for the duration of the call, so reuse/scratch-backed staging payload buffers are valid there.
...
43. Keep unit tests domain-owned too: `tests/unit/ecs` validates generic ECS infrastructure with test-local components, while scene/content component coverage belongs in `tests/unit/scene`.
...
64. Generated G-buffer material pixel shaders include `mesh/material_ps_authoring.slangi`; generated AVBOIT pass pixel shaders include their matching `avboit/*_ps_authoring.slangi`. Both then include the stable `.bind` virtual include for the selected interface. Shader cook generates deterministic Slang-compatible includes under `material_bind_includes`, resolves authored `.bind` includes to generated content without mutating shader source, and tracks both interface and generated include dependencies for checksum invalidation.
...
67. Renderer frame-target resources and texture-bound descriptor-heap handles must be created in resource/target setup, not lazily from `update()`, `render()`, draw submission, or material pipeline hot paths. CSG interval peel/sample resources are created with deferred targets and renderer resource validation; draw and pipeline code may only consume the existing layouts/handles and fail/assert if setup is missing.
...
77. Linux opt/fin GPU timing files record `Name` timing identities and can contain hash tokens; decode them against `.namesym` data or use logserver output for readable scope names. GPU markers and crash-event labels use the separately stored authored text. For stress-test benchmark runs, prefer the Testing runtime directory as the working directory so `cwd/res` wins over any stale parent `__exec/.../res` mirror.
78. `VK_EXT_descriptor_buffer` is a required renderer capability. `DescriptorBufferManager` owns coherent resource and sampler segments with driver-sized, aligned blocks written through `vkGetDescriptorEXT`/`VkDescriptorGetInfoEXT`; command lists bind those blocks by offset. The global `GpuDescriptorHeap` is the only resource transport, with resource/sampler layouts at sets 0/1 and immutable per-generation TLAS blocks at set 2; there is no Backend-A or classic-descriptor fallback. Core descriptor indexing remains the portability floor for shader heap indexing. `robustBufferAccess` is never enabled, so non-robust descriptor sizes are authoritative. Every pipeline layout is descriptor-buffer-compatible, and ordinary layouts carry push constants only without consuming descriptor sets. The migration is complete: active renderer code must not reintroduce descriptor pools, descriptor-set allocation/update/bind calls, update-after-bind, variable-count descriptors, or another non-bindless fallback. Existing descriptor-buffer tests cover layout and write safety; retain a production-style bind/dispatch/readback regression as the remaining integration-validation follow-up.
...
4. This host's GPU timing measurement floor is ~15 µs; treat sub-floor per-pass slices as inconclusive.
5. Scene/scope coverage for surfel-GI and SW-BVH timing: `gi_test` / `stress_test` exercise `render.surfel_trace`. The per-mesh SW BVH rebuild path (`render.sw_bvh_sort` wrapping `bvhBitonicSort` in `buildMeshSwBvhPrepared`, `impl/ecs_render/raytrace/rt_swbvh.cpp`) only fires when the device naturally selects the software-only route. Collect it on a non-RayQuery adapter or a qualified software Vulkan implementation; a RayQuery-capable run must not be relabeled as software-fallback evidence. The sort scope is placed at the call site, not inside `bvhBitonicSort`, to keep the one-shot self-test sort out of the per-frame average.
...
9. Editing a `.slangi` source does not recook the packed `.vol` on launch. Invalidate the relevant asset stamp/cache and rerun the resource cooker before comparing a shader change.
