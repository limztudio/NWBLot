...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  bindless/material-context/scene constants. Its callback no longer restates that entry batch. Direct callers retain
  native setup by default; caustic clears, temporal accumulation, and the multi-pass resolve/ping-pong transitions
