...
7. Opt build gotcha: the `stress_test_sw_smoke` asset-cook prerequisite (`nwb_skinning_culling_benchmark_assets:opt`) currently **crashes the `resource_cooker` with SIGTRAP** in `PrepareGraphicsVolumeAssets` (`impl/assets_graphics/volume_prepare.cpp`, commit `30fe1068c`) while cooking `large_mesh_many_meshlets.nwb` — a pre-existing cooker bug, unrelated to the timing work. It blocks a plain `ninja` opt build. Workaround: compile `rt_swbvh.cpp.o` + `stress_test_project.cpp.o` and relink via the `CXX_EXECUTABLE_LINKER__nwb_stress_test_sw_smoke_opt` edge in `CMakeFiles/impl-opt.ninja` directly (re-archive the `.o` into `libecs_render.a` first), skipping the cook — assets are already cooked in the runtime `.vol`.
8. Phase B collapse (commit `794612f2`): `bvhBitonicSort`'s Phase B merge loop now splits each `sequenceSize > GROUP_SIZE` into inter-tile GLOBAL steps (`compareDistance >= GROUP_SIZE`, one dispatch each — partners cross tiles) plus ONE `GLOBAL_TAIL` groupshared dispatch for the 8 intra-tile steps (`compareDistance = GROUP_SIZE/2..1` — partners stay in-tile). New `mode == GLOBAL_TAIL` shader regime mirrors the Phase A `LOCAL_TILE` collapse. Win: ~9 dispatches + ~9 UAV barriers per sequenceSize drop to 2 (1 GLOBAL + 1 GLOBAL_TAIL). The ascending flag `(global & sequenceSize)` is uniform per GROUP_SIZE-aligned tile. **Status: implemented; pending self-test (dbg, `paddedCount=1024` exercises sequenceSize 512 and 1024) + a fresh opt A/B against the item-6 baseline.**
