...
bool RendererSystem::appendFrameGraph(Core::Telemetry::FrameGraphBuilder& builder){
    ...
    if(m_shadowPrepareTaskGraphValid){
        ...
        const Core::GpuTaskGraphTelemetryOptions shadowPrepareTelemetryOptions{
            .queueAssignments = &m_shadowPrepareTaskGraphQueueAssignments,
        };
        if(!m_shadowPrepareTaskGraph.appendFrameGraphTelemetry(
            builder,
            m_shadowPrepareTaskGraphAnalysis,
            scratchArena,
            shadowPrepareTelemetryOptions
        ))
    ...
    if(m_avboitTaskGraphValid){
        ...
        const Core::GpuTaskGraphTelemetryOptions avboitTelemetryOptions{
            .queueAssignments = &m_avboitTaskGraphQueueAssignments,
        };
        if(!m_avboitTaskGraph.appendFrameGraphTelemetry(
            builder,
            m_avboitTaskGraphAnalysis,
            scratchArena,
            avboitTelemetryOptions
        ))
            NWB_LOGGER_WARNING(NWB_TEXT("RendererSystem: AVBOIT graph telemetry export failed"));
    }
