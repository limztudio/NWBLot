...
producer's temporal-jitter floor (photon Monte-Carlo + 2x temporal-reuse EMA), which concentrates
in the caustic footprint. A correct 4b diff is therefore indistinguishable from a two-identical-run
noise-floor diff -- diffuse grain in the caustic/lighting features. A structured, hard-edged band
(a non-uniform-indexing miss reading another mesh's geometry along a wave-tile boundary) is the
failure signature and would push the max FAR above the floor. Writes an amplified diff PNG.
