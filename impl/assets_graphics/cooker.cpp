...
bool GraphicsAssetCooker::cookGraphicsAssets(const GraphicsCookEnvironment& environment, GraphicsCookResult& outResult){
    ...
    if(!__hidden_cooker::PrepareShaderEntriesForCook(
        ...
        return false;
    if(!__hidden_cooker::AddPlannedFileCount(static_cast<u64>(parsedMetadata.materialEntries.size()), preparedPlan.plannedFileCount))
        return false;
    if(!__hidden_cooker::AddPlannedFileCount(static_cast<u64>(parsedMetadata.meshEntries.size()), preparedPlan.plannedFileCount))
        return false;
    if(!__hidden_cooker::AddPlannedFileCount(
        static_cast<u64>(parsedMetadata.skinnedMeshEntries.size()),
        preparedPlan.plannedFileCount
    ))
        return false;
    if(!__hidden_cooker::ValidateMaterials(
        shaderCook,
    ...
    if(!__hidden_cooker::ConfigureVolumeSizing(preparedPlan.plannedFileCount, volumeConfig))
        return false;

    const __hidden_cooker::StagedVolumePaths stagedVolumePaths = __hidden_cooker::BuildStagedVolumePaths(
        resolvedPaths.outputDirectory,
        volumeConfig.volumeName,
        configurationSafeName,
        scratchArena
    );
    if(!Core::Filesystem::EnsureEmptyStagedDirectory(stagedVolumePaths.stageDirectory, __hidden_cooker::s_CookerLogPrefix, "stage directory"))
        return false;
    Core::Filesystem::StagedDirectoryCleanupGuard stageDirectoryCleanup(stagedVolumePaths.stageDirectory, __hidden_cooker::s_CookerLogPrefix);
    if(!Core::Filesystem::RemoveStagedDirectoryIfPresent(stagedVolumePaths.backupDirectory, __hidden_cooker::s_CookerLogPrefix, "backup directory"))
        return false;
    ...
    usize stagedSegmentCount = 0;
    {
        ...
        if(!__hidden_cooker::AppendPreparedShadersToVolume(
            m_arena,
            shaderCook,
            resolvedPaths.cacheDirectory,
            configurationSafeName,
        ...
        if(!__hidden_cooker::AppendBuiltAssetsToVolume<Material, MaterialAssetCodec>(
            NWB_TEXT("material"),
            parsedMetadata.materialEntries,
            volumeSession,
            seenVirtualPathHashes,
            ...
            return false;
        if(!__hidden_cooker::AppendBuiltAssetsToVolume<Mesh, MeshAssetCodec>(
            NWB_TEXT("mesh"),
            ...
            return false;
        if(!__hidden_cooker::AppendBuiltAssetsToVolume<SkinnedMesh, SkinnedMeshAssetCodec>(
    ...
    if(!outResult.volumeName.assign(__hidden_cooker::s_VolumeName)){
