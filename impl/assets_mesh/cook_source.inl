...
static bool ParseSkinInfluenceStream(
    ...
    if(!skin || !skin->isMap() || skin->asMap().empty()){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedMesh mesh meta '{}': 'skin' must be a non-empty map")
            , PathToString<tchar>(nwbFilePath)
        );
        return false;
    }
    ...
    if(!joints || !joints->isList() || !weights || !weights->isList()){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedMesh mesh meta '{}': 'skin' requires 'joints0' and 'weights0' lists")
            , PathToString<tchar>(nwbFilePath)
        );
        return false;
    }
    ...
    if(jointList.empty() || jointList.size() != weightList.size()){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedMesh mesh meta '{}': skin streams must be non-empty and match")
            , PathToString<tchar>(nwbFilePath)
        );
        return false;
    }
    ...
    for(usize i = 0u; i < jointList.size(); ++i){
        ...
        if(!ParseU16Tuple(nwbFilePath, jointList[i], jointLabel, influence.joint, scratchArena))
            return false;
        if(!ParseMetadataF32Tuple(nwbFilePath, weightList[i], s_SkinnedMeshMetaKind, weightLabel, influence.weight, scratchArena))
            return false;
...
static bool ParseSourceSkinnedMeshMeta(
    ...
    SourceMeshStreams streams(outEntry.positions.get_allocator().arena(), scratchArena);
    if(!ParseCommonSourceMeshStreams(
        discoveredFile,
        asset,
    ...
    if(outEntry.skeletonJointCount == 0u){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedMesh mesh meta '{}': 'skeleton_joint_count' is required for skinned mesh")
    ...
    if(outEntry.inverseBindMatrices.size() != outEntry.skeletonJointCount){
        NWB_LOGGER_ERROR(NWB_TEXT("SkinnedMesh mesh meta '{}': 'inverse_bind_matrices' is required and must match skeleton_joint_count")
            , PathToString<tchar>(discoveredFile.filePath)
        );
        return false;
    }
