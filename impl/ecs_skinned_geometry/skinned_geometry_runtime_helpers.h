...
[[nodiscard]] inline SkinnedGeometryJointMatrix StoreJointMatrix(const SIMDMatrix& matrix){
    SkinnedGeometryJointMatrix result{};
...
[[nodiscard]] inline bool BuildJointPaletteFromSkeletonPose(
    const SkinnedGeometrySkeletonPoseComponent& pose,
    JointMatrixVector& outJointPalette,
    u32& outSkinningMode){
    outJointPalette.clear();
    outSkinningMode = SkinnedGeometrySkinningMode::LinearBlend;

    if(!HasSkeletonPose(&pose))
        return true;
    if(!ValidSkinnedGeometrySkinningMode(pose.skinningMode))
        return false;
    ...
    for(usize jointIndex = 0u; jointIndex < jointCount; ++jointIndex){
        ...
        if(parentJoint != s_SkinnedGeometrySkeletonRootParent){
...
[[nodiscard]] inline SkinnedGeometryJointMatrix StoreJointDualQuaternionPayload(const SIMDVector real, const SIMDVector dual){
    SkinnedGeometryJointMatrix result{};
