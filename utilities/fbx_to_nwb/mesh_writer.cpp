...
namespace __hidden_mesh_writer{
...
bool WriteNwbMesh(
    ...
    for(const Vec3& position : mesh.positions){
        file << "    ";
        __hidden_mesh_writer::WriteVec3(file, position);
        file << ",\n";
    }
    file << "];\n\n";
    ...
    for(const Vec3& normal : mesh.normals){
        file << "    ";
        __hidden_mesh_writer::WriteVec3(file, normal);
        file << ",\n";
    }
    file << "];\n\n";
    ...
    for(const Vec4& tangent : mesh.tangents){
        file << "    ";
        __hidden_mesh_writer::WriteVec4(file, tangent);
        file << ",\n";
    }
    file << "];\n\n";
    ...
    for(const Vec2& uv0 : mesh.uv0){
        file << "    ";
        __hidden_mesh_writer::WriteVec2(file, uv0);
        file << ",\n";
    }
    file << "];\n\n";
    ...
    for(const Vec4& color : mesh.colors){
        file << "    ";
        __hidden_mesh_writer::WriteVec4(file, color);
        file << ",\n";
    }
    file << "];\n\n";
    ...
    if(writeSkinnedMesh){
        ...
        for(const MeshSkinInfluence& influence : mesh.skin){
            file << "        ";
            __hidden_mesh_writer::WriteSkinJoints(file, influence);
            file << ",\n";
        }
        file << "    ],\n";
        file << "    \"weights0\": [\n";
        for(const MeshSkinInfluence& influence : mesh.skin){
            file << "        ";
            __hidden_mesh_writer::WriteSkinWeights(file, influence);
            file << ",\n";
        }
        file << "    ],\n";
        file << "};\n\n";
    ...
    for(const SourceVertexRef& ref : mesh.vertexRefs){
        file << "    ";
        __hidden_mesh_writer::WriteVertexRef(file, ref, writeSkinnedMesh);
        file << ",\n";
    }
    file << "];\n\n";
    ...
    if(writeSkinnedMesh){
        ...
        for(const MeshJointMatrix& matrix : inverseBindMatrices){
            file << "    ";
            __hidden_mesh_writer::WriteJointMatrix(file, matrix);
            file << ",\n";
        }
        file << "];\n\n";
    }
