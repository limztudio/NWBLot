...
[[nodiscard]] inline __m256 Vector2TransformCoordRefinedReciprocalAvx2(__m256 W)noexcept{
...
[[nodiscard]] inline Vector Vector2TransformCoordRefinedReciprocalAvx2TailSse(Vector W)noexcept{
...
inline Float2* MathCallConv Vector2TransformCoordStreamAvx2
(
            ...
            if(OutputStride == sizeof(Float2)){
                if(!(reinterpret_cast<uintptr_t>(pOutputStream) & 0x1F) &&
                    ...
                    if(inputAligned){
                        for(size_t j = 0; j < four; ++j){
                            ...
                            vTempA = _mm256_mul_ps(vTempA, Vector2TransformCoordRefinedReciprocalAvx2(W));

                            W = _mm256_shuffle_ps(vTempA2, vTempA2, _MM_SHUFFLE(3, 3, 3, 3));
                            vTempA2 = _mm256_mul_ps(vTempA2, Vector2TransformCoordRefinedReciprocalAvx2(W));
                ...
                if(inputAligned){
                    for(size_t j = 0; j < four; ++j){
                        ...
                        _mm_storel_epi64(reinterpret_cast<__m128i*>(pOutputVector),
                            _mm_castps_si128(_mm256_castps256_ps128(vTempA2)));
                        pOutputVector += OutputStride;
                        ...
                        _mm_storel_epi64(reinterpret_cast<__m128i*>(pOutputVector),
                            _mm_castps_si128(_mm256_extractf128_ps(vTempA2, 1)));
                        pOutputVector += OutputStride;
    ...
    if(i < VectorCount){
        ...
        for(; i < VectorCount; ++i){
            ...
            vTemp = _mm_mul_ps(vTemp, Vector2TransformCoordRefinedReciprocalAvx2TailSse(W));
