...
[[nodiscard]] inline Vector Vector2TransformCoordRefinedReciprocalSse(Vector W)noexcept{
...
inline Float2* MathCallConv Vector2TransformCoordStreamSse
(
            ...
            if(OutputStride == sizeof(Float2)){
                if(!(reinterpret_cast<uintptr_t>(pOutputStream) & 0xF) &&
                    ...
                    if(inputAligned){
                        for(size_t j = 0; j < two; ++j){
                            ...
                            Vector V1 = _mm_mul_ps(vTemp, Vector2TransformCoordRefinedReciprocalSse(W));
                            ...
                            Vector V2 = _mm_mul_ps(vTemp, Vector2TransformCoordRefinedReciprocalSse(W));
                ...
                if(inputAligned){
                    for(size_t j = 0; j < two; ++j){
                        ...
                        vTemp = _mm_mul_ps(vTemp, Vector2TransformCoordRefinedReciprocalSse(W));
