...
CTest target names remain stable across this layout. Source-only CTest directories do not become launcher commands; add a terminal `launch.py` only for an independently runnable workflow. The current root-launcher A/B commands are `async-shadow-m4`, `bindless-parity`, `command-ir`, `frame-lagged-async-lighting`, `hybrid-shadow-boundary`, `soft-transparent-shadow-fold`, and `transfer-queue`.
