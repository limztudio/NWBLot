...
[[nodiscard]] static Float3U NormalizeOrZero(const Float3U& value){
    ...
    if(!IsFinite(lengthSquared) || lengthSquared <= SkinnedGeometryValidation::s_Epsilon)
...
[[nodiscard]] static Float3U BuildMeshletFaceNormal(
    ...
    const GeometryMeshletDesc& meshlet,
...
template<typename CookEntryT>
static GeometryMeshletBounds BuildMeshletBounds(const CookEntryT& entry, const GeometryMeshletDesc& meshlet){
...
static bool BuildMeshlets(
    ...
    entry.meshlets.reserve((indices.size() / 3u + s_GeometryMaxMeshletTriangles - 1u) / s_GeometryMaxMeshletTriangles);
    ...
    GeometryMeshletDesc current;
    Core::Assets::AssetVector<u32> localVertexRefs(entry.positions.get_allocator().arena());
    localVertexRefs.reserve(s_GeometryMaxMeshletVertices);
    ...
    for(usize indexOffset = 0u; indexOffset < indices.size(); indexOffset += 3u){
        ...
        if(
            ...
            && (
                localVertexRefs.size() + missingCount > s_GeometryMaxMeshletVertices
                || current.primitiveCount + 1u > s_GeometryMaxMeshletTriangles
            )
        )
            flushMeshlet();
        ...
        for(const u32 vertexRefIndex : triangleRefs){
            ...
            if(!found){
                if(localVertexRefs.size() >= s_GeometryMaxMeshletVertices){
