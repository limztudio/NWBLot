...
60. Current material metadata fields are defined by `impl/assets_material/metadata.h`: `interface`, `shaders`, `shader_variant`, `parameters`, `transparent`, and `two_sided`. There is no current `asset.alpha` metadata field; alpha belongs in typed material parameters or shader output. Legacy fields such as `compiler`, `default_variant`, and `instance_override` are validation failures.
