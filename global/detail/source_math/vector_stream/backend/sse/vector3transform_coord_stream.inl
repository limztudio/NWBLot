...
[[nodiscard]] inline Vector Vector3TransformCoordRefinedReciprocalSse(Vector W)noexcept{
...
inline Float3* MathCallConv Vector3TransformCoordStreamSse
(
            ...
            if(OutputStride == sizeof(Float3)){
                if(!(reinterpret_cast<uintptr_t>(pOutputStream) & 0xF) &&
                    ...
                    if(inputAligned){
                        for(size_t j = 0; j < four; ++j){
                            ...
                            vTemp = MATH_FMADD_PS(Z, column2, column3);
                            vTemp = MATH_FMADD_PS(Y, column1, vTemp);
                            vTemp = MATH_FMADD_PS(X, column0, vTemp);

                            W = MATH_PERMUTE_PS(vTemp, _MM_SHUFFLE(3, 3, 3, 3));

                            V2 = _mm_mul_ps(vTemp, Vector3TransformCoordRefinedReciprocalSse(W));
                            ...
                            V3 = _mm_mul_ps(vTemp, Vector3TransformCoordRefinedReciprocalSse(W));
                            ...
                            V4 = _mm_mul_ps(vTemp, Vector3TransformCoordRefinedReciprocalSse(W));
                ...
                if(inputAligned){
                    for(size_t j = 0; j < four; ++j){
                        ...
                        vTemp = _mm_mul_ps(vTemp, Vector3TransformCoordRefinedReciprocalSse(W));
