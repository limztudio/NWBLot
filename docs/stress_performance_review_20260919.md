...
**Latest retained result, 2026-09-20:** the current stress scene contains **20 highpoly bodies: 10 opaque and 10 transparent**. At 1280x900 on Adreno X2-90, the Windows ARM64 `opt` build measures **64.1101 FPS fixed** and **60.4749 FPS aggregated across three rotating runs** (60.4164, 60.6261 and 60.3822 FPS). Each process uses five seconds warmup and thirty seconds of accepted native presentation counting. AVBOIT, reflection, refraction, caustics, shadows and surfel GI settings remain enabled and unchanged. Current raster uses persistent indexed geometry on this device. These are average rates: half-second moving windows reach as low as 54.6750 FPS, so a sixty-FPS floor and long-duration thermal stability are not established. Existing open-body optical artifacts remain. The latest evidence is `__artifacts/stress_60fps/surfel_gather_prefetch_candidate/qualification.json` and the last two retained optimization sections below. Earlier sections are chronological records, including the historical ten-body workload and rejected experiments.
...
At this initial measurement, `stress_test_project.cpp` created **10 bodies: five transparent and five opaque**, plus the surrounding box. This differs from the requested 20-body workload. Results below apply only to the checked-in 10-body scene; they must not be presented as 20-body performance.
...
## Rejected decoded-normal shadow cache and repeat qualification, 2026-09-20

The retained fused-wavelet build repeats at 60.3920 and 60.2982 FPS in motion, alongside its first 60.1121-FPS acquisition. All three use the same frozen runtime, twenty bodies and unchanged settings. These averages exceed sixty, but do not remove the previously reported short-window dips.

A subsequent shader-only experiment decoded each shared normal once during tile fill and stored float3 for reuse across all 25 taps. All 84 exact native cases pass, including 16 new cases with oblique normals across both hemispheres, grazing dot products on both sides of zero, real half input conversion, partial groups and each moments-valid combination. Offline extraction from both frozen packed volumes verifies four 144-element Workgroup arrays and two control barriers in each shader; the normal array changes from half2 to float3. Logical storage rises from 2,592 to 3,744 bytes. The SPIR-V has no ArrayStride decoration, so actual driver allocation is not established.

The cache's moving acquisitions measure 60.4531 and 59.9929 FPS; transparent resolve increases to 1.6550/1.6545 ms, compared with 1.6125 ms before and 1.6311 ms in the subsequent unchanged control. It provides no demonstrated improvement. The production shader is restored byte for byte and assets recooked. All 15 native shadow tests pass again with the restored shader and the expanded 84-case fixture. Only the additional regression cases are retained.

Evidence: `__artifacts/stress_60fps/shadow_wavelet_normals_candidate/` retains the rejected frozen runtime, paired native results, repeated moving acquisitions and packed-module disassembly; `shadow_wavelet_candidate/run20_moving_repeat` and `run20_moving_control` retain the unchanged controls.

## Rejected screen-reflection descent reuse, 2026-09-20

An isolated SSR experiment separated ray advancement from mip refinement, reusing the exact projected position during descents at an unchanged parameter. It retained cell-exit arithmetic, hierarchy fetches, global step accounting, finest-hit validation, fallback and quality settings. A new fixture tests nine actual depth-pyramid scenes, ten budgets and zero/one/full mip selectors: 270 paired dispatches and 7,830 exact raw ray-result comparisons. The frozen classifier now resolves a frozen adjacent screen-trace header, closing the previous shared-header oracle gap. All eight reflection/refraction native tests pass, including the existing 78 classifier cases and their queue/counter/feedback checks.

Despite equivalent outputs, the moving scene regresses to 55.4554 FPS /18.0325 ms, and classification rises to 2.7004 ms from the retained control's 1.3984 ms. Other measured feature ranges remain near their controls. The nested-loop transform is rejected; the production header is restored byte for byte and assets recooked. All eight native tests pass again after restoration. Only the independent frozen header, direct traversal fixture and their registration/provenance are retained. This measurement does not identify the driver-level cause of the slowdown.

Evidence: `__artifacts/stress_60fps/reflection_screen_descent_candidate/` preserves frozen candidate assets, original/restored native logs, timings and GPU summary, corrected-source copies and offline disassembly. Its runtime is a rejected experiment, not the retained performance build.

## Surfel row-head prefetch and final twenty-body average qualification, 2026-09-20

The shared surfel-gather helper now fetches the five immutable hash heads for each `(dy,dz)` row before following its dependent lists. It consumes those heads in the original increasing `dx` order. All 125 cell visits, collision multiplicity, 16-node walk limits, node admission, SH reconstruction, half accumulation, normalization and coverage decisions remain unchanged. Live resolve and the immutable previous-frame bounce snapshot retain their existing producer/read boundaries. There is no zero-weight skip, deduplication, extra GPU resource or sampling/quality change.

The new native target reuses the validation-enabled descriptor-buffer fixture. Twenty-four fields with 65 queries cover partial workgroups, empty/dead/unsampled/opposed/far nodes, hash collisions, 15/16/17-node lists, cycles, cap-hidden NaNs, independent index policies, confidence/distance/normal/SH mixtures, bias, tiny radii, coverage thresholds and nonfinite/half-overflow propagation. All 2,990 candidate/reference comparisons pass exact coverage and RGB-half words, plus output guards and independent activity/constant-field checks. All 449 enabled renderer CPU tests also pass, with 61 existing disabled tests. The earlier retained shadow and reflection suites pass 15 and 8 tests respectively.

Packed production-module inspection verifies the five logical independent head loads precede list traversal. SPIR-V retains a five-iteration loop with an Unroll hint and a Function `uint[5]`; it does not contain five already-unrolled load instructions. Module size grows 11,924 to 12,420 bytes, with no Workgroup variables/barriers and unchanged floating-point opcode counts. This proves emitted control flow, not native register allocation, spills or simultaneous memory requests.

| Measurement | Retained combined-wavelet baseline | Surfel prefetch |
| --- | ---: | ---: |
| Fixed accepted presentations/s | 63.8370 | 64.1101 |
| Fixed wall interval | 15.6649 ms | 15.5982 ms |
| Rotating run 1 | 60.1121 FPS | 60.4164 FPS |
| Rotating run 2 | 60.3920 FPS | 60.6261 FPS |
| Rotating run 3 | 60.2982 FPS | 60.3822 FPS |
| Aggregate rotating rate | 60.2674 FPS | 60.4749 FPS |
| Aggregate rotating interval | 16.5927 ms | 16.5358 ms |
| Fixed surfel resolve | 1.0325 ms | 0.9593 ms |
| Rotating surfel resolve, runs 1/2/3 | 0.8906/0.8874/0.8899 ms | 0.8201/0.8270/0.8240 ms |

The final moving aggregate counts 5,444 presentations over 90.020876 seconds across three separate processes. All runs preserve twenty bodies, 1280x900 and effect settings; validation is excluded during timing. Local resolve savings are consistent, while whole-frame gains are small because work overlaps. Do not sum async scopes. Sixty FPS is now exceeded on average, with half-second windows ranging 54.6750–64.4836 FPS and 64 of 180 windows below 60. A sixty-FPS floor or continuous thermal endurance is not claimed.

The frame 120 full-scene capture passes GPU validation and was visually inspected. Difference from the preceding combined-wavelet image averages 0.08510/255, maximum 33/255; no structural visual difference was observed, and pre-existing noisy open-body glass remains. Shader parity and pass activity do not certify complete optical correctness.

Evidence: `__artifacts/stress_60fps/surfel_gather_prefetch_candidate/` contains frozen runtime/assets and all eight source files, native/CPU logs, three moving runs plus fixed timing, GPU summaries, `qualification.json`, the validated capture/comparison and packed-module inspection. `surfel_gather_prefetch_prepare/` retains reviewed manifests and the frozen original helper. The rejected normal-cache and nested SSR experiments are excluded from production.
