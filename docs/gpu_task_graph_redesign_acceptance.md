...
- The normal deferred Shadow Visibility task now declares its descriptor-visible hardware and software traversal
  ...
  traversal/material streams, TLAS/backing storage, and its output/scratch UAVs. Its graph callback and first
  soft-shadow geometry downsample no longer reissue those entry transitions. Direct or minimal callers retain
  native setup by default; the preflight-failure `CopyDest` clear and later intra-task soft/hybrid UAV fences remain
  native ownership.
