...
static void TestRestSpaceHoleEditCreatesPerInstancePatch(TestContext& context){
    ...
    NWB_ECS_GRAPHICS_TEST_CHECK(context, bottomDepthVertexCount == 4u);
    NWB_ECS_GRAPHICS_TEST_CHECK(
        context,
...
static void TestSurfaceEditStateReplayRestoresMultipleAccessories(TestContext& context){
    ...
    NWB_ECS_GRAPHICS_TEST_CHECK(
        context,
        CommitRecordedHoleAccessory(editedInstance, 14u, 0.48f, 0.25f, 0.16f, 0.18f, state, secondAccessory)
    );
