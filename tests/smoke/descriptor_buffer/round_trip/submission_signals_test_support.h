...
// This recreates the lifetime interval where cancellation and lifecycle drain must wait for exact native resolution instead of retiring the binary semaphore while the submitting thread still owns its raw VkSemaphore handle.
...
// The production queue submits one VkSubmitInfo2 whose final signal is its tracking timeline. Split that submit into an executable command batch and a signal-only tail blocked by a test-owned timeline. This leaves the real command buffer and timestamps complete while the production token remains naturally incomplete.
