...
[[nodiscard]] inline __m256 Vector3TransformCoordRefinedReciprocalAvx2(__m256 W)noexcept{
    const __m256 two = _mm256_set1_ps(2.0f);
    __m256 reciprocal = _mm256_rcp_ps(W);
    reciprocal = _mm256_mul_ps(reciprocal, _mm256_fnmadd_ps(W, reciprocal, two));
    reciprocal = _mm256_mul_ps(reciprocal, _mm256_fnmadd_ps(W, reciprocal, two));
    return reciprocal;
}

[[nodiscard]] inline Vector Vector3TransformCoordRefinedReciprocalAvx2TailSse(Vector W)noexcept{
...
inline Float3* MathCallConv Vector3TransformCoordStreamAvx2
(
                ...
                if(!(reinterpret_cast<uintptr_t>(pOutputStream) & 0xF) &&
                    ...
                    if(inputAligned){
                        for(size_t j = 0; j < four; ++j){
                            ...
                            const __m256 w12 = _mm256_shuffle_ps(result12, result12, _MM_SHUFFLE(3, 3, 3, 3));
                            result12 = _mm256_mul_ps(result12, Vector3TransformCoordRefinedReciprocalAvx2(w12));
                            ...
                            const __m256 w34 = _mm256_shuffle_ps(result34, result34, _MM_SHUFFLE(3, 3, 3, 3));
                            result34 = _mm256_mul_ps(result34, Vector3TransformCoordRefinedReciprocalAvx2(w34));
    ...
    if(i < VectorCount){
        ...
        for(; i < VectorCount; ++i){
            ...
            Vector W = MATH_PERMUTE_PS(result, _MM_SHUFFLE(3, 3, 3, 3));
            result = _mm_mul_ps(result, Vector3TransformCoordRefinedReciprocalAvx2TailSse(W));
