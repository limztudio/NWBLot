...
- The normal deferred Software Caustics task now consumes graph-declared descriptor-visible shared inputs: sampled
  ...
  photon producer, geometry downsample, and shared wavelet resolver are distinct merged callbacks in that same
  caustics packet: geometry starts the retained full-resolve timing interval, while wavelet declares the exact
  accumulator range and resolve-geometry cache as `ShaderResource`. The compiler therefore lowers both in-packet
  `UnorderedAccess`-to-`ShaderResource` handoffs before wavelet records, which closes that interval. When no photon
  producer records, geometry and wavelet emit no dispatch and the graph-owned black irradiance clear remains final.
  Packet-identity validation retains the established timing and acceptance endpoint. Direct callers retain native
  setup by default, while dynamic history/half/irradiance resolve-buffer transitions remain native task-local
  ownership. The normal graph also no longer repeats unchanged world/depth sampled-state setup across each wavelet
  pass.
...
- The normal deferred Hardware Caustics task now consumes graph-declared descriptor-visible static producer inputs:
  ...
  reasserting them natively. The selected photon producer, geometry downsample, and shared wavelet resolver are
  distinct merged callbacks in that same caustics packet: geometry starts the retained full-resolve timing interval,
  while wavelet declares the exact accumulator range and resolve-geometry cache as `ShaderResource`. The compiler
  therefore lowers both in-packet `UnorderedAccess`-to-`ShaderResource` handoffs before wavelet records, which
  closes that interval. When no photon producer records, geometry and wavelet emit no dispatch and the graph-owned
  black irradiance clear remains final. Packet-identity validation retains the established timing and acceptance
  endpoint. Direct callers retain native setup by default; dynamic history/half/irradiance resolve-buffer
  transitions remain native task-local ownership. The normal graph also no longer repeats unchanged world/depth
  sampled-state setup across each wavelet pass.
