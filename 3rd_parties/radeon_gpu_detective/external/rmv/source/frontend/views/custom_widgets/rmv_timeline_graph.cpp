...
void RMVTimelineGraph::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    Q_ASSERT(config_.model_data != nullptr);
    if (!config_.model_data->IsTimelineGenerationInProgress())
    {
        painter->save();
        ...
        int  bucket_index      = 0;
        bool success           = true;
        int  num_bucket_groups = config_.model_data->GetNumBucketGroups();
        if (num_bucket_groups > 0)
        {
            do
            {
                for (int bucket_group_number = 0; bucket_group_number < num_bucket_groups; bucket_group_number++)
                {
                    int    bucket_group_index = config_.model_data->RemapBucketGroupNumberToIndex(bucket_group_number);
                    QColor color              = config_.colorizer->GetColor(bucket_group_index);
                    qreal  y_pos              = 0.0f;
                    qreal  height             = 0.0f;
                    success = config_.model_data->GetHistogramData(bucket_group_number, bucket_index, config_.model_data->GetNumBucketGroups(), y_pos, height);
                    if (success)
                    {
                        QRectF rect;
