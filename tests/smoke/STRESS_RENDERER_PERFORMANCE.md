...
## Mesh kernel qualification infrastructure

The common native `nwb_mesh_kernel_tests` fixture cooks the live production compute-emulation entrypoint and a frozen original entrypoint through the production shader cooker. Its two tests contain 84 paired cases covering local attribute seams, 8/16/32-bit packed references, one/two meshlets, identity/transformed/resolved-deformed streams, culling and excess-workgroup guards. Every generated 64-byte vertex must match exactly; distinct initial sentinel patterns also detect missing writes and writes outside the expected output. This tests resolved vertex generation, not execution of skinning or the CSG-enabled kernel.

Opt and Dbg builds and actual validation-backed native runs passed all 84 cases with no skips. At this qualification commit, the live shader is still the original implementation, so these results establish fixture self-parity, not candidate correctness or a speedup. Logs, XML and copies of the actual emitted modules are under `__artifacts/stress_60fps/step20/native_fixture_self_*` and `native_fixture_self_modules_v1/`. The initial CMake regeneration overlapped an active build and encountered a Ninja log lock; the subsequent serialized regeneration and both builds passed.

Run `ctest --test-dir __cmake/build/windows-clang-arm64 -C opt -R '^nwb_mesh_kernel_tests$' --output-on-failure` (or `-C dbg`). The candidate reads the configured checkout's live shader source at execution time. Preserve the emitted files from `tests/smoke/mesh_kernel/kernels/<config>` in the build directory before another run. This fixture supplements packed-volume shader verification, optical smoke captures and frame-time measurements.
