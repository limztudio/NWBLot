...
struct SkinningGraphStateProbeTask{
    ...
    [[nodiscard]] static bool record(
        ...
        for(usize expectationIndex = 0u; expectationIndex < payload.expectationCount; ++expectationIndex){
            ...
            Buffer* const buffer = context.taskGraph.bufferForResource(expectation.resource);
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedShadowVisibilityEntryStatesRecordWithoutNativeBridge){
    ...
    const GpuSubmissionPacketId preparePacket = compiledGraph.packetForTask(prepareTask);
    ...
    const GpuCompiledTask* const compiledShadow = compiledGraph.findTask(shadowTask);
    ASSERT_NE(compiledShadow, nullptr);
    const GpuCompiledBarrier* const shadowBarriers = compiledGraph.taskPrologueBarriers(shadowTask);
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedSoftTransparentTraceToResolveRecordsWithoutNativeBridge){
    ...
    EXPECT_EQ(compiledGraph.packetForTask(transparentTemporalMergeTask), packet);
    ...
    EXPECT_EQ(compiledGraph.packet(packet).taskCount, 7u);
    const GpuCompiledTask* const compiledTrace = compiledGraph.findTask(traceTask);
    ASSERT_NE(compiledTrace, nullptr);
    const GpuCompiledBarrier* const traceBarriers = compiledGraph.taskPrologueBarriers(traceTask);
    ASSERT_NE(traceBarriers, nullptr);
    ...
    const GpuCompiledTask* const compiledOpaqueFirstWavelet = compiledGraph.findTask(opaqueFirstWaveletTask);
    ASSERT_NE(compiledOpaqueFirstWavelet, nullptr);
    const GpuCompiledBarrier* const opaqueFirstWaveletBarriers = compiledGraph.taskPrologueBarriers(opaqueFirstWaveletTask);
    ...
    const GpuCompiledTask* const compiledTransparentTemporalMerge = compiledGraph.findTask(transparentTemporalMergeTask);
    ASSERT_NE(compiledTransparentTemporalMerge, nullptr);
    const GpuCompiledBarrier* const transparentTemporalMergeBarriers =
        compiledGraph.taskPrologueBarriers(transparentTemporalMergeTask)
    ;
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedSoftwareCausticsEntryStatesRecordWithoutNativeBridge){
    ...
    const GpuSubmissionPacketId irradianceClearPacket = compiledGraph.packetForTask(irradianceClearTask);
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedNonTemporalCausticAccumulatorClearRecordsBeforePhotonProducer){
    ...
    ASSERT_TRUE(compiler.compile(graph, analysis, topology, assignments, compiledGraph, scratchArena));
    ASSERT_EQ(compiledGraph.packetCount(), 1u);
    const GpuSubmissionPacketId clearPacket = compiledGraph.packetForTask(clearTask);
    ...
    const GpuCompiledTask* const compiledClear = compiledGraph.findTask(clearTask);
    ...
    const GpuCompiledBarrier* const clearBarriers = compiledGraph.taskPrologueBarriers(clearTask);
    ...
    for(u32 barrierIndex = 0u; barrierIndex < compiledClear->prologueBarrierCount; ++barrierIndex){
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedCausticPhotonGeometryPrepareFiveWaveletAndUpsampleHandoffsRecordWithoutNativeBridge){
    ...
    const GpuSubmissionPacketId preparePacket = compiledGraph.packetForTask(prepareTask);
    const GpuSubmissionPacketId waveletPacket = compiledGraph.packetForTask(waveletTask);
    ...
    for(u32 barrierIndex = 0u; barrierIndex < compiledWavelet->prologueBarrierCount; ++barrierIndex){
    ...
    for(u32 barrierIndex = 0u; barrierIndex < compiledSecondWavelet->prologueBarrierCount; ++barrierIndex){
    ...
    for(u32 barrierIndex = 0u; barrierIndex < compiledThirdWavelet->prologueBarrierCount; ++barrierIndex){
    ...
    for(u32 barrierIndex = 0u; barrierIndex < compiledFourthWavelet->prologueBarrierCount; ++barrierIndex){
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedSurfelGiResolveRecordsWithoutNativeBridge){
    ...
    const auto hasAgeFreeTransition = [&](const GpuGraphResourceId resource, const ResourceStates::Mask before, const ResourceStates::Mask after){
        for(u32 barrierIndex = 0u; barrierIndex < compiledAgeFree->prologueBarrierCount; ++barrierIndex){
    ...
    const GpuCompiledTask* const compiledSpawn = compiledGraph.findTask(spawnTask);
    ASSERT_NE(compiledSpawn, nullptr);
    const GpuCompiledBarrier* const spawnBarriers = compiledGraph.taskPrologueBarriers(spawnTask);
    ...
    const auto hasSpawnBarrier = [&](const GpuCompiledBarrierType::Enum type, const GpuGraphResourceId resource, const ResourceStates::Mask before, const ResourceStates::Mask after){
        for(u32 barrierIndex = 0u; barrierIndex < compiledSpawn->prologueBarrierCount; ++barrierIndex){
    ...
    const GpuCompiledTask* const compiledTraceBuildArgs = compiledGraph.findTask(traceBuildArgsTask);
    ASSERT_NE(compiledTraceBuildArgs, nullptr);
    const GpuCompiledBarrier* const traceBuildArgsBarriers = compiledGraph.taskPrologueBarriers(traceBuildArgsTask);
    ...
    const auto hasTraceBuildArgsBarrier = [&](const GpuCompiledBarrierType::Enum type, const GpuGraphResourceId resource, const ResourceStates::Mask before, const ResourceStates::Mask after){
        for(u32 barrierIndex = 0u; barrierIndex < compiledTraceBuildArgs->prologueBarrierCount; ++barrierIndex){
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedAvboitOccupancyCsgIntervalSampleStateRecordsWithoutNativeBridge){
    ...
    ASSERT_TRUE(compiler.compile(graph, analysis, topology, assignments, compiledGraph, scratchArena));
    ASSERT_EQ(compiledGraph.packetCount(), 1u);
    const GpuSubmissionPacketId packet = compiledGraph.packetForTask(preTask);
    ASSERT_TRUE(packet.valid());
    ...
    ASSERT_EQ(packetPlan.taskCount, 5u);
    ASSERT_NE(compiledGraph.packetTasks(packet), nullptr);
    EXPECT_EQ(compiledGraph.packetTasks(packet)[0u], preTask);
    EXPECT_EQ(compiledGraph.packetTasks(packet)[1u], spanTask);
    EXPECT_EQ(compiledGraph.packetTasks(packet)[2u], combineTask);
    EXPECT_EQ(compiledGraph.packetTasks(packet)[3u], clearTask);
    EXPECT_EQ(compiledGraph.packetTasks(packet)[4u], occupancyTask);

    const GpuCompiledTask* const compiledPre = compiledGraph.findTask(preTask);
    ...
    ASSERT_NE(compiledClear, nullptr);
    ASSERT_NE(compiledOccupancy, nullptr);
    ASSERT_EQ(compiledPre->prologueStateSeedCount, 0u);
    ASSERT_EQ(compiledSpan->prologueStateSeedCount, 0u);
    ...
    EXPECT_TRUE(hasTextureTransition(preBarriers, compiledPre->prologueBarrierCount, capBackNormalResource, peelRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
    EXPECT_TRUE(hasTextureTransition(preBarriers, compiledPre->prologueBarrierCount, intervalDepthResource, peelRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
    ...
    EXPECT_TRUE(hasTextureTransition(spanBarriers, compiledSpan->prologueBarrierCount, receiverSpanCountResource, receiverSpanCountRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
    EXPECT_TRUE(hasTextureUav(combineBarriers, compiledCombine->prologueBarrierCount, capBackNormalResource, peelRange));
    EXPECT_TRUE(hasTextureUav(combineBarriers, compiledCombine->prologueBarrierCount, intervalDepthResource, peelRange));
    EXPECT_TRUE(hasTextureUav(combineBarriers, compiledCombine->prologueBarrierCount, intervalIdResource, peelRange));
    EXPECT_TRUE(hasTextureUav(combineBarriers, compiledCombine->prologueBarrierCount, receiverSpanDataResource, receiverSpanRange));
    EXPECT_TRUE(hasTextureUav(combineBarriers, compiledCombine->prologueBarrierCount, receiverSpanCountResource, receiverSpanCountRange));
    EXPECT_TRUE(hasTextureTransition(combineBarriers, compiledCombine->prologueBarrierCount, removedIntervalDepthResource, removedIntervalRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
    EXPECT_TRUE(hasTextureTransition(combineBarriers, compiledCombine->prologueBarrierCount, removedIntervalCapNormalResource, removedIntervalRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
    EXPECT_TRUE(hasTextureTransition(combineBarriers, compiledCombine->prologueBarrierCount, removedIntervalDataResource, removedIntervalRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
    EXPECT_TRUE(hasTextureTransition(combineBarriers, compiledCombine->prologueBarrierCount, removedIntervalCountResource, removedIntervalCountRange, ResourceStates::Common, ResourceStates::UnorderedAccess));
...
TEST_F(DescriptorBufferRoundTripTest, GraphOwnedSkinningDeformationHandoffStaysInGraph){
    ...
    EXPECT_EQ(finalizerAssignment->queue, primaryGraphicsQueue);
    const GpuSubmissionPacketId deformationPacket = compiledGraph.packetForTask(deformationTask);
    const GpuSubmissionPacketId postDispatchPacket = compiledGraph.packetForTask(postDispatchTask);
    const GpuSubmissionPacketId finalizerPacket = compiledGraph.packetForTask(finalizerTask);
    ...
    const GpuCompiledTask* const compiledFinalizer = compiledGraph.findTask(finalizerTask);
    ASSERT_NE(compiledDeformation, nullptr);
    ASSERT_NE(compiledPostDispatch, nullptr);
    ASSERT_NE(compiledFinalizer, nullptr);
    EXPECT_EQ(compiledDeformation->prologueBarrierCount, 3u);
    EXPECT_EQ(compiledPostDispatch->prologueBarrierCount, 4u);
    EXPECT_EQ(compiledFinalizer->prologueBarrierCount, 3u);
    const GpuCompiledBarrier* const postDispatchBarriers = compiledGraph.taskPrologueBarriers(postDispatchTask);
    const GpuCompiledBarrier* const finalizerBarriers = compiledGraph.taskPrologueBarriers(finalizerTask);
