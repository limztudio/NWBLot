...
## Step14: cull non-opaque hardware shadow candidates - not retained
...
## Shared preparation: native reflection-normal edge coverage

The existing spatial-kernel native test now covers 256 internal scenarios, retaining its original 160 and adding 80 matrix plus 16 literal 3x1 cases. New inputs cover invalid neighbors with different consuming receivers, invalid centers, NaN/Inf and overflow, adjacent finite normalization thresholds, and valid non-unit normal lengths. Authored normal alpha is ignored. Existing copy, diagnostic-alpha, finite-output and actual-filtering checks remain; independent literal output ranges prevent a common wrong fallback from passing merely because two outputs agree. The frozen reference shader is unchanged.

This test-only preparation was qualified against the unchanged production shader in both Opt and Dbg. Each run executed both native GTests with no skipped or failed cases: 256 spatial scenarios and 39 unchanged depth scenarios. XML descendant failures/skips and run/completed state were checked explicitly; the hardcoded XML errors field is not treated as a GPU validation count. The existing validation-backed fixture retains resources through submission/completion and compares exact RGBA16 words using each readback's row pitch. Opt and Dbg builds passed without compiler warning/error markers.

The three emitted native modules and test executable were copied immediately after each successful run, before another source/configuration could replace them. The native executable compiles from its baked live checkout path; copying it alone does not freeze future shader inputs. Post-run hashes confirm the common test and all six original shader/reference/fixture dependencies. No rendering behavior or performance improvement is claimed by this preparation.

The original two-file proposal remains preserved. The common test manifest is `f5fa9c1c2c16f2b73f6d4ef606d3d6b3047338db2f7df398f8ff89f32a4a38d0`; the subsequent shader-only manifest is `13eeb725c80a3ca4388853aa612df66294531acb0912ba1969c9af18e9a8a8c7`. Their exact split patches reconstruct the original patch. The test is common to both future timing arms; only `impl/assets/graphics/reflection/spatial_cs.slang` is the optimization delta. Build logs, native XML/logs, copied modules and source/qualification records are under `__artifacts/reflection_optimization_followup/step15/correctness_baseline_common_v1/`.


## Step15: cache decoded spatial normals - not retained

The candidate is correct on the qualified native and scene inputs, but its measured whole-frame changes do not meet the original practical threshold. Both original timing reports are `unresolved`, with all unaffected controls equivalent. The shader was restored and the ordinary runtime recooked in Opt and Dbg. The restored optimized executable/dependencies and complete authored runtime match the original frozen arm. The common 256-case native regression coverage remains.

The one-file shader experiment decoded and normalized each cooperatively loaded normal in FP32, storing validity in the unused shared w lane. Invalid centers retained positive-Y fallback; invalid neighbors retained the consuming receiver's resolved normal. Normal alpha was ignored. No shared allocation, barrier, descriptor, material, precision gate or filter order changed. Active tiles may normalize inputs whose consumers subsequently reject them, so moving arithmetic alone was not treated as a performance result.

Both configurations built and cooked all 218 ordinary assets without compiler warning/error markers. Baseline and candidate each passed two actual native GTests in Opt and Dbg, with no skipped, disabled or failed tests: 256 spatial and 39 depth internal scenarios per run. Exact RGBA16 comparisons, independent fallback/activity checks and the validation-backed fixture were preserved. Native modules and executables were archived immediately after each run. The reference/depth modules and same-configuration executable identities match across arms; each arm's spatial native modules match its actual packed module.

Both arms passed the unchanged 14 roughness, 12 temporal and 22 general reflection captures: 96 actual GPU-debug captures in total. Each raw log contains all three actual validation startup markers exactly once and no original strict failure tokens. All five temporal reset/fresh pairs are exact within each arm. As an additional observation, 46 of 48 corresponding BMPs are identical across arms. The two eight-sample rough/spatial exceptions use graphics source frames 10 and 11 in opposite order between arms; their original quality oracles pass, but those differences are not attributed to the shader optimization.

Actual Vulkan-validated SPIR-V places center/halo normal reciprocal-square-root operations before the cache barrier, outside the nested filter loops; receiver view-direction normalization remains. Four 196-entry float4 arrays and three Workgroup barriers are unchanged. The complete volumes contain 218 payloads and 164 shader records, changing only spatial bytecode and its index. The module grows from 23,572 to 24,248 bytes and the authored volume by 676 bytes. Exactly one source path differs and executable/dependency inventories are identical. This establishes emitted placement and bounded correctness, not final driver ISA or universal numerical behavior.

| Workload | Baseline GPU frame ms | Candidate | Paired change, 95% interval (ms) | Original status |
| --- | ---: | ---: | --- | --- |
| Rough spatial | 4.160327 | 4.135067 | -0.025259 [-0.047230, -0.003700] | `unresolved` |
| Mirror spatial | 3.358457 | 3.343120 | -0.015337 [-0.024137, -0.005946] | `unresolved` |

Observed frame reductions are 0.61% and 0.46%, below the original practical bounds of 0.124810 and 0.100754 ms. The rough spatial scope falls from 0.802670 to 0.764277 ms, paired interval [-0.049425, -0.029691] ms. Mirror spatial is 0.060549 to 0.060636 ms, interval [-0.000855, +0.001041] ms. The rough scope observation does not override the predeclared whole-frame retention rule. No regression or broad speedup claim is inferred from these unresolved primary results.

All 32 planned launches completed across two eight-block balanced campaigns. Each launch retained six publications after two warm-up publications and passed the minimum 100 completed-frame gate. Actual retained GPU frames total 11,291 for rough and 13,884 for mirror, 25,175 overall. Every trial remains; there were no failed acquisitions, exclusions, retries or overlapping GPU/build/capture work. Original confidence, control and practical gates were unchanged.

Evidence is preserved under `__artifacts/reflection_optimization_followup/step15/`: split source manifests, predeclared plan, both physical arms, native qualification, 96 raw captures and independent reviews, actual emitted/whole-volume reviews, `timing_execution_v1` with every trial and original report, and restoration logs/identities. Shader-only proposal manifest SHA256 is `13eeb725c80a3ca4388853aa612df66294531acb0912ba1969c9af18e9a8a8c7`. The subsequent radius experiment must start from the restored original normalization implementation, preserving only the common expanded tests.


## Shared preparation: production spatial-radius transitions

A default-off smoke controller now exercises the real renderer settings, pipeline preparation and task snapshot through radii 1, 3, 2 and back to 1. Six selections compare fresh starts at each radius against the corresponding transition prefixes. Each phase joins accepted history, exact seed/reset source and completed spatial timing; the final readback source must belong to a dense completed spatial window. It adds no production API or shader change and no new timing scope. Ordinary reflection and timing runs leave the controller disabled.

All 17 parser/image/validation-marker tests passed, and the actual smoke executable built cleanly in Opt and Dbg. Both configurations passed six GPU-debug owner captures against the unchanged dynamic shader. All three sequence-to-fresh comparisons are exact over 691,200 pixels. Within the projected 213,120-pixel mirror footprint, radius pairs differ by more than two byte levels at 95,497, 137,978 and 131,378 pixels, respectively, exceeding the fixed 213-pixel sensitivity floor. Actual startup validation markers and complete source/binary/runtime/logger identities are checked by the owning runner.

This proves bounded production selection and return-to-radius output behavior. It does not expose private cache-hit counts or test live resizing, device recreation, allocation failure or concurrent snapshot retention. The later native specialized-kernel tests remain separate. The preparation is common to both Step16 arms; no performance gain is attributed to it. Source follows `.helper`, including named namespaces, include order and CRLF source files.

Evidence is under `__artifacts/reflection_optimization_followup/step16/owner_preparation_checks_v1`, the two `owner_baseline_*_v1` freezes and twelve `owner_baseline_*_captures_v1` capture records. Effective package manifest SHA256 is `09da02076e220f1af0a00838ef81bc8395f7f96191c05e4904843d57fc69a759`; its dependency-only reconciliation preserves all seven reviewed payloads. The original Step16 package is preserved, and the optimization is explicitly rebased over Step15's restored original normalization implementation.


## Shared correction: shadow warm-up source contract

The broader Step16 unit run exposed one stale source-contract expectation: the shadow smoke readiness log was corrected in shared commit `023d47fb9` from "rendered frames" to "update callbacks", while its unit expectation still used the old phrase. The test now expects the actual message and its comment distinguishes callback warm-up from completed GPU history. Fixed timestep, 360-callback readiness, suspension behavior and all image/timing gates are unchanged; no renderer or smoke behavior changes.

The original failed optimized run is retained in `step16/correctness_candidate_v1`: 380 active ECS tests passed, one failed, while all three native reflection GTests had already passed 464 spatial and 39 depth cases. After the common test-only correction and final `.helper` call formatting, both Opt and Dbg passed all 381 active ECS tests with no runtime skips or failures. The existing 61 disabled tests remain disabled and are not counted as executed qualification. Initial and final correction evidence is preserved in `step16/common_shadow_contract_fix_v1/v2`.

Before any Step16 timing, the original seven candidate paths were temporarily restored. Fresh baseline source freezes include this common correction and reuse the byte-identical, already qualified original smoke executable/runtime; only this unit-test source and report documentation differ from the previous baseline source inventory. The candidate then reapplies the same seven reviewed payloads. Earlier freezes and the failed unit run remain intact; no result is overwritten or relabeled.


## Step16: specialize spatial radius and retain prepared pipelines

Retained under the original plan's separate criterion for a bounded reduction in shared memory, with matched quality and no practical whole-frame regression under the original bound. Both original timing reports remain `unresolved`: neither establishes the original 3% practical frame improvement. This does not convert the secondary scope result into a qualified whole-renderer speedup.

The reflection postprocess owner now lazily retains three shader/pipeline pairs, selected by the exact radius 1/2/3 archive signatures. Snapshots and graph payloads retain the selected strong pipeline handle and matching settings. Invalid radii reject before indexing, zero/disabled filtering bypasses spatial preparation, and invalidation clears every bank. This stays in the reflection resource owner and uses the existing shader loader and task-lifetime contracts. Retaining up to three pipelines trades bounded pipeline memory and first-use preparation for specialized shaders; private cache-hit counts, live resize/device recreation and concurrent invalidation were not experimentally qualified.

| Variant | Tile extent | Four shared float4 arrays | Existing eligibility scalar |
| --- | --- | ---: | ---: |
| Dynamic 0 | Up to 14x14 | 12,544 bytes | 4 bytes |
| Radius 1 | 10x10 | 6,400 bytes | 4 bytes |
| Radius 2, default setting | 12x12 | 9,216 bytes | 4 bytes |
| Radius 3 | 14x14 | 12,544 bytes | 4 bytes |

The default radius reduces array storage by 3,328 bytes, 26.53%; radius 1 reduces it by 6,144 bytes, 48.98%. These are verified SPIR-V Workgroup allocations, not measurements of final driver occupancy or device-wide memory use. Macro zero remains the dynamic reference, not a fixed filter radius zero. Its actual module and disassembly are byte-identical to the former default. Production selects the three specialized variants. The rejected Step15 normal-cache changes are absent; original normal decoding and receiver-specific fallbacks remain.

Both Opt and Dbg built/cooked 221 assets without compiler warning/error markers. Each passed three actual native GTests containing 464 spatial and 39 depth cases: the common 256 dynamic cases, 180 specialized matrix cases, 12 specialized literal cases and 16 retained-pipeline switches. Exact RGBA16 outputs, independent fallback/activity checks and validation-backed resource/readback lifetimes remain. All 381 active ECS tests passed in both configurations; 61 existing disabled tests are reported separately. The initial unrelated stale shadow-contract failure and common correction are preserved and described above. No candidate capture or timing was acquired before that correction was qualified.

The production owner separately passed six captures in each configuration, joining accepted phases, exact reset/source frames and completed spatial timing. Three return-to-radius images are exact against their fresh starts, and all twelve candidate owner BMPs are identical to their corresponding original owner baseline BMPs. The fixed radius-discrimination gates remain satisfied. The candidate also passed all 48 original optimized reflection scene captures: 14 roughness, 12 temporal and 22 general. Every owner and ordinary capture log contains all three actual GPU-debug markers and no original strict failure tokens. Native pipeline switches and actual renderer-owner transitions qualify distinct boundaries.

All four actual optimized packed modules validate for Vulkan 1.2, preserve the push/descriptor ABI, disjoint center/halo writers and three Workgroup barriers. Their sizes are 23,572 / 23,016 / 23,032 / 23,048 bytes for variants 0/1/2/3. Candidate native Opt and Dbg modules match these four packed modules; their unchanged reference remains 20,280 bytes. The complete optimized volume grows from 218 to 221 payloads and 164 to 167 shader records, replacing one spatial selector with four and updating the index. No unrelated shader or nonshader contents change; 206 unchanged payloads move in the layout. Volume growth is 69,897 bytes. Complete packed comparison is Opt-specific; Dbg has separate native and actual owner qualification.

Specialization lets the compiler fold/reassociate the FP32 spatial denominator: for example radius 2 uses `8 * (roughness * roughness)` where the dynamic shader computes through its radius-dependent sigma. There is no new source precision mode, and all tested half outputs and owner images match, but this does not establish identical floating-point intermediates for every input or driver. The actual emitted review records this distinction rather than asserting universal bitwise equivalence.

| Workload | Baseline GPU frame ms | Candidate | Paired change, 95% interval (ms) | Original status |
| --- | ---: | ---: | --- | --- |
| Rough spatial | 4.161531 | 4.069737 | -0.091795 [-0.104520, -0.078697] | `unresolved` |
| Mirror spatial | 3.349256 | 3.340746 | -0.008510 [-0.017044, +0.001061] | `unresolved` |

The observed rough frame reduction is 2.21%, below its 0.124846 ms practical bound. Its spatial scope falls from 0.801171 to 0.710914 ms, paired interval [-0.099754, -0.081146] ms, an observed 11.27% reduction. Mirror spatial is 0.060027 to 0.059910 ms, interval [-0.001555, +0.001534] ms; it is unresolved. Both whole-frame confidence intervals satisfy non-regression against the original practical bounds, and all three unaffected controls are equivalent in each workload. The mirror frame's practical bound is 0.100478 ms. No primary gate, control tolerance or confidence interval was relaxed for retention.

All 32 planned timing launches completed across two eight-block balanced campaigns, retaining six publications after two warm-ups and at least 100 completed frames per trial. Actual measured GPU frames total 11,354 rough plus 13,893 mirror, 25,247 overall. All planned trials remain, with no acquisition failure, exclusion or retry and no overlapping GPU/build/capture work. Earlier unit qualification failure is distinct from timing acquisition.

Evidence remains under `__artifacts/reflection_optimization_followup/step16/`: original and radius-only rebased packages, predeclared plans, original/fresh common baseline freezes, candidate v2 freezes, native/owner/48-scene evidence and reviews, complete volume/emitted evidence, and `timing_execution_v2`. The selected-module comparison is the sibling `step16_emitted_compare_v2`; its initial in-step output location was rejected by the helper's input-overlap preflight before writing output, then corrected without changing helper bytes or shader gates. Effective seven-file manifest SHA256 is `957cef0e8cc020c4c088dfdf6f42cef4a9cb446dbfafd589ec813374aa9d0ef6`. Shared `.nwb` metadata remains declarative, and authored C++/shader files follow `.helper` with CRLF and named namespaces.

## Step 17 common qualification support

Added a caustic-owned native fixture that cooks the unchanged production shader and an independently frozen reference through the production ShaderCook. Two GTests exercise 99 wavelet scenarios at steps 1, 2 and 4, including partial tiles, zero and invalid geometry, and invalid right neighbors whose raw coordinates still control filter spacing. Every output RGBA16 component is compared exactly. The unchanged baseline passes both GTests in Opt and Dbg. The first Opt run exposed an overly strict auxiliary ideal constant-color assertion: 453 pixels produced the immediate predecessor half value in each RGB channel, while all production/reference comparisons passed. The original failure and emitted modules are preserved. Before any candidate run, the common test was corrected to allow the adjacent half codes for that auxiliary magnitude check and require exact power-of-two channel ratios; exact reference equality and all 99 scenarios remain unchanged.

Opt-in smoke support adds caustic photon/resolve timing scopes and fixed populated/sparse camera presets, keeping ordinary smoke defaults. The new footprint qualifier retains real enabled/disabled framebuffer captures and checks a finite, visible ground receiver; the A/B runner replays and revalidates the raw qualification evidence before timing. These are visible footprint roles, not measurements of internal wavelet tile occupancy. All 57 parser tests pass. Caustic and ordinary transparent smoke variants build in Opt and Dbg; both configurations pass 381 active ECS tests with 61 pre-existing disabled tests. Actual footprint/combined-scene qualification and candidate evaluation follow separately.

Evidence: `__artifacts/reflection_optimization_followup/step17/{native_baseline_common_v1,native_baseline_common_v2,uniform_oracle_correction_v1,measurement_common_v1,evaluation_plan_v5.json}`. Measurement source is the reviewed `caustic_measurement/proposal_format_v1` package. All original performance gates remain in force; this preparation makes no optimization or speed claim.

### Common caustic qualification launcher correction
