...
| Hybrid fallback target-scene benchmark arm: renderer build and Vulkan/X11 stress capture | passed; paired healthy and persistent-fallback targets use the same fixed-yaw ten-character animated hybrid-shadow scene. The fallback arm forces the optional traversal tail to miss every frame, restores the retained immutable hardware context, and suppresses only repeated test-loop diagnostics so target timing is not dominated by log traffic. The fallback capture passed; target-hardware timing evidence remains to be collected through the paired workflow. |
