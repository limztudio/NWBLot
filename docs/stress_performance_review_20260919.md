...
The shared surfel-gather helper now fetches the five immutable hash heads for each `(dy,dz)` row before following its dependent lists. It consumes those heads in the original increasing `dx` order. All125 cell visits, collision multiplicity, 16-node walk limits, node admission, SH reconstruction, half accumulation, normalization and coverage decisions remain unchanged. Live resolve and the immutable previous-frame bounce snapshot retain their existing producer/read boundaries. There is no zero-weight skip, deduplication, extra GPU resource or sampling/quality change.

The new native target reuses the validation-enabled descriptor-buffer fixture. Twenty-four fields with65 queries cover partial workgroups, empty/dead/unsampled/opposed/far nodes, hash collisions, 15/16/17-node lists, cycles, cap-hidden NaNs, independent index policies, confidence/distance/normal/SH mixtures, bias, tiny radii, coverage thresholds and nonfinite/half-overflow propagation. All2,990 candidate/reference comparisons pass exact coverage and RGB-half words, plus output guards and independent activity/constant-field checks. All449 enabled renderer CPU tests also pass, with61 existing disabled tests. The earlier retained shadow and reflection suites pass15 and8 tests respectively.

Packed production-module inspection verifies the five logical independent head loads precede list traversal. SPIR-V retains a five-iteration loop with an Unroll hint and a Function `uint[5]`; it does not contain five already-unrolled load instructions. Module size grows11,924 to12,420 bytes, with no Workgroup variables/barriers and unchanged floating-point opcode counts. This proves emitted control flow, not native register allocation, spills or simultaneous memory requests.
...
| Rotating run1 | 60.1121 FPS | 60.4164 FPS |
| Rotating run2 | 60.3920 FPS | 60.6261 FPS |
| Rotating run3 | 60.2982 FPS | 60.3822 FPS |
...
| Rotating surfel resolve, run1/2/3 | 0.8906/0.8874/0.8899 ms | 0.8201/0.8270/0.8240 ms |

The final moving aggregate counts5,444 presentations over90.020876 seconds across three separate processes. All runs preserve twenty bodies,1280x900 and effect settings; validation is excluded during timing. Local resolve savings are consistent, while whole-frame gains are small because work overlaps. Do not sum async scopes. Sixty FPS is now exceeded on average, with half-second windows ranging54.6750–64.4836 FPS and64 of180 windows below60. A sixty-FPS floor or continuous thermal endurance is not claimed.

The frame120 full-scene capture passes GPU validation and was visually inspected. Difference from the preceding combined-wavelet image averages0.08510/255, maximum33/255; no structural visual difference was observed, and pre-existing noisy open-body glass remains. Shader parity and pass activity do not certify complete optical correctness.
