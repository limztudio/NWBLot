...
// NWB modification: adapts the historical private miniz API to the shared miniz package.
...
int png_decoder::decompress_line(uint32_t* bytes_decoded)
{
	...
	for (; ; )
	{
		...
		for (; ; )
		{
			...
			// The repository's shared miniz exposes the zlib-compatible mz_inflate()
			// entry point, rather than Basis Universal's private mz_inflate2() extension.
			status = buminiz::mz_inflate(&m_inflator, buminiz::MZ_NO_FLUSH);
